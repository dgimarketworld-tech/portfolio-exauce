...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../cartes/index.php" class="nav-link active"><span class="nav-icon">💳</span> Cartes</a>
...
$cartes=DB::all("SELECT ca.*,co.numero as compte_num,u.first_name,u.last_name FROM cartes ca JOIN comptes co ON ca.compte_id=co.id JOIN users u ON co.user_id=u.id ORDER BY ca.cree_le DESC LIMIT 100");
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrf($_POST['_csrf']??'')){
    $cid=(int)($_POST['cid']??0); $action=$_POST['action']??'';
    if($cid&&in_array($action,['block','unblock'])){
        DB::update("UPDATE cartes SET statut=:s WHERE id=:id",['s'=>$action==='block'?'bloquee':'active','id'=>$cid]);
        showToast('Carte '.($action==='block'?'bloquée':'débloquée'),'success');
...
<div class="admin-card">
  <table class="admin-table"><thead><tr><th>Numéro masqué</th><th>Client</th><th>Type</th><th>Réseau</th><th>Expire</th><th>Statut</th><th>Actions</th></tr></thead>
  <tbody>
  <?php foreach($cartes as $c): ?>
  <tr>
    <td style="font-family:monospace"><?php echo htmlspecialchars($c['numero_masque'],ENT_QUOTES,'UTF-8');?></td>
    <td><?php echo htmlspecialchars(($c['first_name']??'').' '.($c['last_name']??''),ENT_QUOTES,'UTF-8');?></td>
    <td><span class="badge badge-blue"><?php echo ucfirst($c['type']);?></span></td>
    <td><?php echo strtoupper($c['reseau']);?></td>
    <td><?php echo date('m/y',strtotime($c['expire_le']));?></td>
    <td><span class="badge badge-<?php echo $c['statut']==='active'?'green':($c['statut']==='bloquee'?'red':'blue');?>"><?php echo ucfirst($c['statut']);?></span></td>
    <td>
      <form method="POST" style="display:inline">
        ...
        <button type="submit" class="admin-btn-xs admin-btn-danger" onclick="return confirm('Bloquer cette carte ?')">Bloquer</button>
        <?php else: ?>
        <input type="hidden" name="action" value="unblock"/>
        <button type="submit" class="admin-btn-xs admin-btn-success">Débloquer</button>
        <?php endif; ?>
      </form>
    </td>
  </tr>
  <?php endforeach; ?>
  <?php if(empty($cartes)): ?><tr><td colspan="7" style="text-align:center;padding:2rem;color:var(--gray400)">Aucune carte</td></tr><?php endif; ?>
  </tbody></table>
</div>
