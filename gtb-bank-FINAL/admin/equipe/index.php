...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../equipe/index.php" class="nav-link active"><span class="nav-icon">👤</span> Équipe</a>
...
if ($_SERVER['REQUEST_METHOD']==='POST' && Security::csrfCheck($_POST['_csrf']??'')) {
    ...
    if ($action==='add') {
        ...
        $pass  = $_POST['password']??'';
        if (!$fn||!$ln||!$email||!$pass) { $error='Tous les champs sont requis.'; }
        elseif (DB::scalar("SELECT COUNT(*) FROM admins WHERE email=:e",['e'=>$email])>0) { $error='Cet email est déjà utilisé.'; }
        else {
            DB::insertInto('admins',['email'=>$email,'password_hash'=>password_hash($pass,PASSWORD_ARGON2ID,['memory_cost'=>65536,'time_cost'=>4,'threads'=>2]),'first_name'=>$fn,'last_name'=>$ln,'role'=>$role,'status'=>'active','is_active'=>1,'created_at'=>date('Y-m-d H:i:s')]);
            $success='Membre '.$fn.' '.$ln.' ajouté avec succès.';
        }
    } elseif ($action==='toggle' && ($adm['role']??'')==='superadmin') {
        $id=(int)($_POST['id']??0);
        if ($id!==$currentAdmin['id']) {
            $m=DB::one("SELECT status FROM admins WHERE id=:id",['id'=>$id]);
            $ns=$m['status']==='active'?'suspended':'active';
            DB::update("UPDATE admins SET status=:s WHERE id=:id",['s'=>$ns,'id'=>$id]);
            $success='Statut mis à jour.';
        }
    } elseif ($action==='delete' && ($adm['role']??'')==='superadmin') {
        $id=(int)($_POST['id']??0);
        if ($id!==$currentAdmin['id']) {
            DB::update("DELETE FROM admins WHERE id=:id",['id'=>$id]);
            $success='Membre supprimé.';
        }
    } elseif ($action==='role' && ($adm['role']??'')==='superadmin') {
        $id=(int)($_POST['id']??0);
        $role=$_POST['role']??'support';
        if ($id!==$currentAdmin['id']) {
            DB::update("UPDATE admins SET role=:r WHERE id=:id",['r'=>$role,'id'=>$id]);
            $success='Rôle mis à jour.';
        }
    }
}

$membres = DB::all("SELECT id,first_name,last_name,email,role,status,last_login_at,created_at FROM admins ORDER BY created_at ASC");
$total   = count($membres);
$actifs  = count(array_filter($membres,fn($m)=>$m['status']==='active'));
?>

<div class="page-header">
  <div class="page-header-top">
    <div><h1 class="page-title">Équipe</h1><p class="page-sub">Gérez les membres de l'équipe d'administration</p></div>
    <div class="page-actions">
      <button class="btn btn-primary" onclick="openModal('modal-add')">+ Ajouter un membre</button>
    </div>
  </div>
</div>
...
<div class="admin-kpi-grid" style="margin-bottom:1.5rem">
  <div class="kpi-card"><div class="kpi-icon" style="background:rgba(59,130,246,.1);color:var(--accent)">👥</div><div><div class="kpi-label">Total membres</div><div class="kpi-value"><?=$total?></div></div></div>
  <div class="kpi-card"><div class="kpi-icon" style="background:rgba(16,185,129,.1);color:var(--success)">✓</div><div><div class="kpi-label">Actifs</div><div class="kpi-value"><?=$actifs?></div></div></div>
  <div class="kpi-card"><div class="kpi-icon" style="background:rgba(245,158,11,.12);color:#B45309">🛡️</div><div><div class="kpi-label">Superadmins</div><div class="kpi-value"><?=count(array_filter($membres,fn($m)=>$m['role']==='superadmin'))?></div></div></div>
</div>

<div class="admin-card">
  <div class="card-header">
    <div><div class="card-title">Membres de l'équipe</div></div>
  </div>
  <div style="overflow-x:auto">
    <table class="admin-table">
      <thead><tr>
        <th>Membre</th><th>Rôle</th><th>Statut</th><th>Dernière connexion</th><th>Membre depuis</th>
        <?php if(($adm['role']??'')==='superadmin'):?><th style="text-align:right">Actions</th><?php endif;?>
      </tr></thead>
      <tbody>
      <?php foreach($membres as $m):
        $ini=strtoupper(substr($m['first_name'],0,1).substr($m['last_name'],0,1));
        $roleColors=['superadmin'=>'badge-danger','admin'=>'badge-blue','support'=>'badge-warning','compliance'=>'badge-purple'];
        $rc=$roleColors[$m['role']]??'badge-gray';
      ?>
      <tr>
        <td><div class="cell-user"><div class="cell-avatar"><?=$ini?></div><div class="cell-user-info"><div style="font-weight:600;color:var(--gray900)"><?=htmlspecialchars($m['first_name'].' '.$m['last_name'])?></div><div class="cell-user-mail"><?=htmlspecialchars($m['email'])?></div></div></div></td>
        <td><span class="badge <?=$rc?>"><?=ucfirst($m['role'])?></span></td>
        <td><span class="badge <?=$m['status']==='active'?'badge-success':'badge-gray'?>"><?=$m['status']==='active'?'Actif':'Suspendu'?></span></td>
        <td style="font-size:.76rem;color:var(--gray500)"><?=$m['last_login_at']?date('d/m/Y H:i',strtotime($m['last_login_at'])):'Jamais'?></td>
        <td style="font-size:.76rem;color:var(--gray500)"><?=date('d/m/Y',strtotime($m['created_at']))?></td>
        <?php if(($adm['role']??'')==='superadmin' && $m['id']!==$currentAdmin['id']):?>
        <td>
          <div class="action-btns">
            <form method="POST" style="display:inline">
              ...
              <select name="role" class="admin-input" style="padding:.28rem .6rem;font-size:.72rem" onchange="this.form.submit()">
                <?php foreach(['superadmin','admin','support','compliance'] as $r):?>
            ...
            <form method="POST" style="display:inline">
              ...
              <input type="hidden" name="id" value="<?=$m['id']?>">
              <button type="submit" class="act-btn <?=$m['status']==='active'?'':'success'?>" title="<?=$m['status']==='active'?'Suspendre':'Réactiver'?>"><?=$m['status']==='active'?'⏸':'▶'?></button>
            </form>
            <form method="POST" style="display:inline" onsubmit="return confirm('Supprimer ce membre ?')">
              <input type="hidden" name="_csrf" value="<?=$csrf?>">
              <input type="hidden" name="_action" value="delete">
              <input type="hidden" name="id" value="<?=$m['id']?>">
              <button type="submit" class="act-btn danger" title="Supprimer">🗑</button>
            </form>
          </div>
        </td>
        <?php elseif(($adm['role']??'')==='superadmin'):?><td><span style="font-size:.72rem;color:var(--gray400)">Vous</span></td><?php endif;?>
      </tr>
      <?php endforeach;?>
      </tbody>
    </table>
...
<div class="modal-overlay" id="modal-add">
  <div class="modal">
    <div class="modal-head">
      <div><div class="modal-title">Ajouter un membre</div><div class="modal-sub">Créer un accès administrateur</div></div>
    ...
    <form method="POST">
      ...
      <div class="modal-body">
        <div class="form-row">
          <div class="form-group"><label class="form-label">Prénom</label><input class="form-input" name="first_name" required placeholder="Jean"></div>
          <div class="form-group"><label class="form-label">Nom</label><input class="form-input" name="last_name" required placeholder="Dupont"></div>
        </div>
        <div class="form-group"><label class="form-label">Email</label><input class="form-input" name="email" type="email" required placeholder="jean@globaltrust-b.com"></div>
        <div class="form-group"><label class="form-label">Mot de passe temporaire</label><input class="form-input" name="password" type="password" required minlength="8"></div>
        <div class="form-group"><label class="form-label">Rôle</label>
          <select class="form-select" name="role">
            <option value="support">Support</option>
            <option value="compliance">Compliance</option>
            <option value="admin">Admin</option>
            <?php if(($adm['role']??'')==='superadmin'):?><option value="superadmin">Superadmin</option><?php endif;?>
          </select>
        </div>
      </div>
