...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../kyc/index.php" class="nav-link active"><span class="nav-icon">🪪</span> KYC</a>
...
$users=DB::all("SELECT id,first_name,last_name,email,kyc_status,kyc_document_type,created_at FROM users WHERE role='user' AND kyc_status=:s ORDER BY created_at DESC LIMIT 50",['s'=>$filter]);
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrf($_POST['_csrf']??'')){
    $uid=(int)($_POST['uid']??0); $action=$_POST['action']??'';
    if($uid&&in_array($action,['verify','reject'])){
        $new=$action==='verify'?'verified':'rejected';
        DB::update("UPDATE users SET kyc_status=:s WHERE id=:id",['s'=>$new,'id'=>$uid]);
        $msg=$new==='verified'?'Votre identité a été vérifiée avec succès.':'Votre dossier KYC a été refusé. Contactez le support.';
        notify($uid,'KYC '.ucfirst($new),$msg,$new==='verified'?'success':'error');
        header('Location: index.php?status='.$filter);exit;
    }
}
$csrf=csrf_token();
?>
<div style="display:flex;gap:.5rem;margin-bottom:1.5rem" class="chips-bar">
  <a href="?status=pending" class="chip <?php echo $filter==='pending'?'active':'';?>">En attente</a>
  <a href="?status=verified" class="chip <?php echo $filter==='verified'?'active':'';?>">Vérifiés</a>
  <a href="?status=rejected" class="chip <?php echo $filter==='rejected'?'active':'';?>">Refusés</a>
</div>
<div class="admin-card">
  <table class="admin-table"><thead><tr><th>Client</th><th>Email</th><th>Inscrit le</th><th>Statut KYC</th><th>Document</th><th>Actions</th></tr></thead>
  <tbody>
  <?php foreach($users as $u): ?>
  <tr>
    <td><b><?php echo htmlspecialchars(($u['first_name']??'').' '.($u['last_name']??''),ENT_QUOTES,'UTF-8');?></b></td>
    <td style="font-size:.78rem"><?php echo htmlspecialchars($u['email'],ENT_QUOTES,'UTF-8');?></td>
    <td style="font-size:.75rem;color:var(--gray400)"><?php echo format_date($u['created_at']);?></td>
    <td><span class="badge badge-<?php echo $u['kyc_status']==='verified'?'green':($u['kyc_status']==='rejected'?'red':'blue');?>"><?php echo ucfirst($u['kyc_status']);?></span></td>
    <td><?php
      $docLabels=['cni_ue'=>'🪪 CNI-UE','passeport'=>'🛂 Passeport','permis'=>'🚗 Permis','autre'=>'📋 Autre'];
      $doc=$u['kyc_document_type']??null;
      if($doc==='cni_ue'):?><span class="badge badge-success">🪪 CNI-UE</span><?php
      elseif($doc): ?><span class="badge badge-danger" title="Document non accepté"><?php echo htmlspecialchars($docLabels[$doc]??$doc,ENT_QUOTES,'UTF-8');?></span><?php
      else: ?><span style="font-size:.72rem;color:var(--gray400)">Non renseigné</span><?php endif; ?>
    </td>
    <td>
      <?php if($u['kyc_status']==='pending'): ?>
      <?php if(($u['kyc_document_type']??'')!=='cni_ue'): ?>
        <span style="font-size:.72rem;color:var(--danger);display:inline-flex;align-items:center;gap:.3rem" title="Document non conforme — seule la CNI-UE est acceptée">⛔ Doc. non conforme</span>&nbsp;
        <form method="POST" style="display:inline"><input type="hidden" name="_csrf" value="<?php echo htmlspecialchars($csrf,ENT_QUOTES,'UTF-8');?>"/><input type="hidden" name="uid" value="<?php echo $u['id'];?>"/><input type="hidden" name="action" value="reject"/><button type="submit" class="admin-btn-xs admin-btn-danger">✕ Refuser</button></form>
      <?php else: ?>
        <form method="POST" style="display:inline"><input type="hidden" name="_csrf" value="<?php echo htmlspecialchars($csrf,ENT_QUOTES,'UTF-8');?>"/><input type="hidden" name="uid" value="<?php echo $u['id'];?>"/><input type="hidden" name="action" value="verify"/><button type="submit" class="admin-btn-xs admin-btn-success">✓ Valider</button></form>
        <form method="POST" style="display:inline"><input type="hidden" name="_csrf" value="<?php echo htmlspecialchars($csrf,ENT_QUOTES,'UTF-8');?>"/><input type="hidden" name="uid" value="<?php echo $u['id'];?>"/><input type="hidden" name="action" value="reject"/><button type="submit" class="admin-btn-xs admin-btn-danger">✕ Refuser</button></form>
      <?php endif; ?>
      <?php else: ?><span style="font-size:.75rem;color:var(--gray400)">—</span><?php endif; ?>
    </td>
  </tr>
  <?php endforeach; ?>
  <?php if(empty($users)): ?><tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--gray400)">Aucun dossier <?php echo $filter;?></td></tr><?php endif; ?>
  </tbody></table>
</div>
