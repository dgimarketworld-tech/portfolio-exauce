...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../credits/index.php" class="nav-link active"><span class="nav-icon">📋</span> Crédits</a>
...
$filter=$_GET['status']??'en_etude';
$credits=DB::all("SELECT cr.*,u.first_name,u.last_name,u.email FROM credits cr JOIN users u ON cr.user_id=u.id WHERE cr.statut=:s ORDER BY cr.cree_le DESC LIMIT 100",['s'=>$filter]);
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrf($_POST['_csrf']??'')){
    $cid=(int)($_POST['cid']??0); $action=$_POST['action']??'';
    if($cid&&in_array($action,['accepte','refuse'])){
        $echeance=$action==='accepte'?date('Y-m-d',strtotime('+1 month')):null;
        DB::update("UPDATE credits SET statut=:s,prochaine_echeance=:e WHERE id=:id",['s'=>$action,'e'=>$echeance,'id'=>$cid]);
        $cr=DB::one("SELECT user_id,reference,type FROM credits WHERE id=:id",['id'=>$cid]);
        if($cr) notify($cr['user_id'],'Crédit '.($action==='accepte'?'accepté':'refusé'),'Votre dossier #'.$cr['reference'].' a été '.($action==='accepte'?'accepté — les fonds seront versés sous 48h':'refusé par notre service').'.', $action==='accepte'?'success':'error');
...
<div class="chips-bar" style="margin-bottom:1.5rem">
  <a href="?status=en_etude" class="chip <?php echo $filter==='en_etude'?'active':'';?>">En étude</a>
  <a href="?status=accepte" class="chip <?php echo $filter==='accepte'?'active':'';?>">Acceptés</a>
  <a href="?status=en_cours" class="chip <?php echo $filter==='en_cours'?'active':'';?>">En cours</a>
  <a href="?status=refuse" class="chip <?php echo $filter==='refuse'?'active':'';?>">Refusés</a>
  <a href="?status=solde" class="chip <?php echo $filter==='solde'?'active':'';?>">Soldés</a>
</div>
<div class="admin-card">
  <table class="admin-table"><thead><tr><th>Référence</th><th>Client</th><th>Type</th><th>Montant</th><th>Mensualité</th><th>Taux</th><th>Durée</th><?php if($filter==='en_etude'):?><th>Actions</th><?php endif;?></tr></thead>
  <tbody>
  <?php foreach($credits as $cr): ?>
  <tr>
    <td style="font-family:monospace;font-size:.75rem"><?php echo htmlspecialchars($cr['reference'],ENT_QUOTES,'UTF-8');?></td>
    <td><b><?php echo htmlspecialchars(($cr['first_name']??'').' '.($cr['last_name']??''),ENT_QUOTES,'UTF-8');?></b></td>
    <td><span class="badge badge-blue"><?php echo ucfirst($cr['type']);?></span></td>
    <td style="font-weight:700"><?php echo format_money($cr['montant']);?></td>
    <td><?php echo format_money($cr['mensualite']);?>/mois</td>
    <td><?php echo $cr['taux'];?>%</td>
    <td><?php echo $cr['duree_mois'];?> mois</td>
    <?php if($filter==='en_etude'): ?>
    <td>
      <form method="POST" style="display:inline"><input type="hidden" name="_csrf" value="<?php echo htmlspecialchars($csrf,ENT_QUOTES,'UTF-8');?>"/><input type="hidden" name="cid" value="<?php echo $cr['id'];?>"/><input type="hidden" name="action" value="accepte"/><button type="submit" class="admin-btn-xs admin-btn-success" onclick="return confirm('Accepter ce crédit ?')">✓ Accepter</button></form>
      <form method="POST" style="display:inline"><input type="hidden" name="_csrf" value="<?php echo htmlspecialchars($csrf,ENT_QUOTES,'UTF-8');?>"/><input type="hidden" name="cid" value="<?php echo $cr['id'];?>"/><input type="hidden" name="action" value="refuse"/><button type="submit" class="admin-btn-xs admin-btn-danger" onclick="return confirm('Refuser ce crédit ?')">✕ Refuser</button></form>
    </td>
    <?php endif; ?>
  </tr>
  <?php endforeach; ?>
  <?php if(empty($credits)): ?><tr><td colspan="8" style="text-align:center;padding:2rem;color:var(--gray400)">Aucun crédit <?php echo $filter;?></td></tr><?php endif; ?>
  </tbody></table>
</div>
