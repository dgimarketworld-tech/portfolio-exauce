...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../logs/index.php" class="nav-link active"><span class="nav-icon">📜</span> Audit Logs</a>
...
$logs=DB::all("SELECT al.*,u.first_name,u.last_name FROM audit_log al LEFT JOIN users u ON al.user_id=u.id ORDER BY al.created_at DESC LIMIT :l OFFSET :o",['l'=>$per,'o'=>$pg['offset']]);
?>
<div class="admin-card">
  <table class="admin-table"><thead><tr><th>Date</th><th>Utilisateur</th><th>Action</th><th>IP</th><th>Détails</th></tr></thead>
  <tbody>
  <?php foreach($logs as $log): ?>
  <tr>
    <td style="font-size:.75rem;white-space:nowrap"><?php echo format_datetime($log['created_at']);?></td>
    <td style="font-size:.8rem"><?php echo $log['user_id']?htmlspecialchars(($log['first_name']??'').' '.($log['last_name']??''),ENT_QUOTES,'UTF-8'):'<span style="color:var(--gray400)">Système</span>';?></td>
    <td><span class="badge badge-blue" style="font-family:monospace;font-size:.7rem"><?php echo htmlspecialchars($log['action'],ENT_QUOTES,'UTF-8');?></span></td>
    <td style="font-family:monospace;font-size:.75rem;color:var(--gray400)"><?php echo htmlspecialchars($log['ip_address']??'—',ENT_QUOTES,'UTF-8');?></td>
    <td style="font-size:.72rem;color:var(--gray500);max-width:200px;overflow:hidden;text-overflow:ellipsis"><?php echo $log['details']?htmlspecialchars(substr($log['details'],0,80),ENT_QUOTES,'UTF-8').'…':'—';?></td>
  </tr>
  <?php endforeach; ?>
  <?php if(empty($logs)): ?><tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--gray400)">Aucun log</td></tr><?php endif; ?>
  </tbody></table>
</div>
<?php if($pg['total_pages']>1): ?>
<div style="display:flex;justify-content:center;gap:.5rem;margin-top:1rem">
  <?php if($pg['has_prev']): ?><a href="?page=<?php echo $pg['current']-1;?>" class="admin-btn">← Préc.</a><?php endif; ?>
  <span style="font-size:.8rem;color:var(--gray400);padding:.5rem">Page <?php echo $pg['current'];?>/<?php echo $pg['total_pages'];?></span>
  <?php if($pg['has_next']): ?><a href="?page=<?php echo $pg['current']+1;?>" class="admin-btn">Suiv. →</a><?php endif; ?>
</div>
<?php endif; ?>
