...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../support/index.php" class="nav-link active"><span class="nav-icon">❓</span> Support</a>
...
$filter=$_GET['status']??'ouvert';
$tickets=DB::all("SELECT t.*,u.first_name,u.last_name,a.first_name as c_fn,a.last_name as c_ln FROM tickets t JOIN users u ON t.user_id=u.id LEFT JOIN admins a ON t.conseiller_id=a.id WHERE t.statut=:s ORDER BY t.cree_le DESC LIMIT 100",['s'=>$filter]);
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrf($_POST['_csrf']??'')){
    $tid=(int)($_POST['tid']??0); $action=$_POST['action']??'';
    if($tid&&in_array($action,['take','resolve','close'])){
        $map=['take'=>'en_cours','resolve'=>'resolu','close'=>'ferme'];
        DB::update("UPDATE tickets SET statut=:s,conseiller_id=:cid,mis_a_jour=NOW() WHERE id=:id",['s'=>$map[$action],'cid'=>Session::adminId(),'id'=>$tid]);
        if($action==='resolve'){$tk=DB::one("SELECT user_id FROM tickets WHERE id=:id",['id'=>$tid]);if($tk)notify($tk['user_id'],'Ticket résolu','Votre demande a été résolue. N\'hésitez pas à nous contacter si besoin.','success');}
        header('Location: index.php?status='.$filter);exit;
    }
}
$csrf=csrf_token();
?>
<div class="chips-bar" style="margin-bottom:1.5rem">
  <a href="?status=ouvert" class="chip <?php echo $filter==='ouvert'?'active':'';?>">Ouverts</a>
  <a href="?status=en_cours" class="chip <?php echo $filter==='en_cours'?'active':'';?>">En cours</a>
  <a href="?status=resolu" class="chip <?php echo $filter==='resolu'?'active':'';?>">Résolus</a>
  <a href="?status=ferme" class="chip <?php echo $filter==='ferme'?'active':'';?>">Fermés</a>
</div>
<div class="admin-card">
  <table class="admin-table"><thead><tr><th>Référence</th><th>Client</th><th>Catégorie</th><th>Sujet</th><th>Priorité</th><th>Conseiller</th><th>Créé le</th><th>Actions</th></tr></thead>
  <tbody>
  <?php foreach($tickets as $t): ?>
  <tr>
    <td style="font-family:monospace;font-size:.75rem">#<?php echo htmlspecialchars($t['reference'],ENT_QUOTES,'UTF-8');?></td>
    <td><?php echo htmlspecialchars(($t['first_name']??'').' '.($t['last_name']??''),ENT_QUOTES,'UTF-8');?></td>
    <td><span class="badge badge-blue"><?php echo ucfirst($t['categorie']);?></span></td>
    <td style="font-size:.82rem;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><?php echo htmlspecialchars($t['sujet'],ENT_QUOTES,'UTF-8');?></td>
    <td><span class="badge badge-<?php echo $t['priorite']==='urgente'?'red':($t['priorite']==='normale'?'blue':'gray');?>"><?php echo ucfirst($t['priorite']);?></span></td>
    <td style="font-size:.78rem"><?php echo $t['c_fn']?htmlspecialchars($t['c_fn'].' '.$t['c_ln'],ENT_QUOTES,'UTF-8'):'<span style="color:var(--gray400)">Non assigné</span>';?></td>
    <td style="font-size:.75rem;color:var(--gray400)"><?php echo time_ago($t['cree_le']);?></td>
    <td>
      <?php if($t['statut']==='ouvert'): ?>
      <form method="POST" style="display:inline"><input type="hidden" name="_csrf" value="<?php echo htmlspecialchars($csrf,ENT_QUOTES,'UTF-8');?>"/><input type="hidden" name="tid" value="<?php echo $t['id'];?>"/><input type="hidden" name="action" value="take"/><button type="submit" class="admin-btn-xs admin-btn-success">Prendre</button></form>
      <?php elseif($t['statut']==='en_cours'): ?>
      <form method="POST" style="display:inline"><input type="hidden" name="_csrf" value="<?php echo htmlspecialchars($csrf,ENT_QUOTES,'UTF-8');?>"/><input type="hidden" name="tid" value="<?php echo $t['id'];?>"/><input type="hidden" name="action" value="resolve"/><button type="submit" class="admin-btn-xs admin-btn-success">✓ Résoudre</button></form>
      <?php endif; ?>
    </td>
  </tr>
  <?php endforeach; ?>
  <?php if(empty($tickets)): ?><tr><td colspan="8" style="text-align:center;padding:2rem;color:var(--gray400)">Aucun ticket</td></tr><?php endif; ?>
  </tbody></table>
</div>
