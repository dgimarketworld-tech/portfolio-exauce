...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../utilisateurs/index.php" class="nav-link active"><span class="nav-icon">👥</span> Utilisateurs</a>
...
<header class="admin-topbar">
  <div style="display:flex;align-items:center;gap:1rem"><button class="sidebar-toggle" id="sidebarToggle">☰</button><h1 style="font-family:'Sora',sans-serif;font-size:1rem;font-weight:700;color:var(--dark)"><?php echo $title??'';?></h1></div>
...
$search=trim($_GET['q']??''); $status_f=$_GET['status']??'';
$where="role='user'"; $params=[];
if($search){$where.=" AND (first_name LIKE :q OR last_name LIKE :q OR email LIKE :q)";$params['q']="%$search%";}
if($status_f&&in_array($status_f,['active','suspended','closed'])){$where.=" AND status=:s";$params['s']=$status_f;}
$total=(int)DB::scalar("SELECT COUNT(*) FROM users WHERE $where",$params);
$per=20; $page=max(1,(int)($_GET['page']??1)); $off=($page-1)*$per;
$params['l']=$per;$params['o']=$off;
$users=DB::all("SELECT * FROM users WHERE $where ORDER BY created_at DESC LIMIT :l OFFSET :o",$params);
// Actions POST
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrfAdmin($_POST['_csrf']??'')){
    $uid=(int)($_POST['uid']??0); $action=$_POST['action']??'';
    if($uid&&in_array($action,['suspend','activate'])){
        $new=$action==='suspend'?'suspended':'active';
        DB::update("UPDATE users SET status=:s WHERE id=:id AND role='user'",['s'=>$new,'id'=>$uid]);
        notify($uid,'Compte '.($new==='active'?'activé':'suspendu'),'Votre compte a été '.($new==='active'?'réactivé':'suspendu').' par un administrateur.',$new==='active'?'success':'warning');
        header('Location: index.php');exit;
    }
}
$csrf=csrf_token_admin()??csrf_token();
?>
<div style="display:flex;gap:1rem;margin-bottom:1.5rem;flex-wrap:wrap;align-items:center">
  <form method="GET" style="display:flex;gap:.5rem;flex:1;min-width:200px">
    <input type="text" name="q" value="<?php echo e($search);?>" placeholder="Nom, email…" class="admin-input" style="flex:1"/>
    <select name="status" class="admin-input" onchange="this.form.submit()">
      <option value="">Tous statuts</option>
      <option value="active" <?php echo $status_f==='active'?'selected':'';?>>Actifs</option>
      <option value="suspended" <?php echo $status_f==='suspended'?'selected':'';?>>Suspendus</option>
    </select>
...
<div class="admin-card">
  <table class="admin-table">
    <thead><tr><th>N° Client</th><th>Nom</th><th>Email</th><th>Plan</th><th>KYC</th><th>Statut</th><th>Inscrit le</th><th>Actions</th></tr></thead>
    <tbody>
    <?php foreach($users as $u): ?>
    <tr>
      <td style="font-family:monospace;font-size:.75rem"><?php echo e($u['client_number']??'—');?></td>
      <td><b><?php echo e(($u['first_name']??'').' '.($u['last_name']??''));?></b></td>
      <td style="font-size:.78rem"><?php echo e($u['email']);?></td>
      <td><span class="badge badge-<?php echo $u['plan']==='premium'?'gold':'blue';?>"><?php echo ucfirst($u['plan']??'standard');?></span></td>
      <td><span class="badge badge-<?php echo ($u['kyc_status']??'pending')==='verified'?'green':(($u['kyc_status']??'pending')==='rejected'?'red':'blue');?>"><?php echo ucfirst($u['kyc_status']??'pending');?></span></td>
      <td><span class="badge badge-<?php echo ($u['status']??'active')==='active'?'green':'red';?>"><?php echo ucfirst($u['status']??'active');?></span></td>
      <td style="font-size:.75rem;color:var(--gray400)"><?php echo format_date($u['created_at']);?></td>
      <td style="display:flex;gap:.4rem;align-items:center">
        <a href="detail.php?id=<?php echo $u['id'];?>" class="admin-btn-xs" style="background:#EFF6FF;color:#3B82F6;border:1px solid #BFDBFE;text-decoration:none">Voir</a>
        <form method="POST" style="display:inline">
          <input type="hidden" name="_csrf" value="<?php echo e($csrf);?>"/>
          <input type="hidden" name="uid" value="<?php echo $u['id'];?>"/>
          <?php if(($u['status']??'active')==='active'):?>
          <input type="hidden" name="action" value="suspend"/>
          <button type="submit" class="admin-btn-xs admin-btn-danger" onclick="return confirm('Suspendre cet utilisateur ?')">Suspendre</button>
          <?php else:?>
          <input type="hidden" name="action" value="activate"/>
          <button type="submit" class="admin-btn-xs admin-btn-success">Activer</button>
          <?php endif;?>
        </form>
      </td>
    </tr>
    <?php endforeach;?>
    <?php if(empty($users)):?><tr><td colspan="8" style="text-align:center;padding:2rem;color:var(--gray400)">Aucun utilisateur trouvé</td></tr><?php endif;?>
    </tbody>
  </table>
</div>
<?php if(ceil($total/$per)>1):?>
<div style="display:flex;justify-content:center;gap:.5rem;margin-top:1rem">
  <?php if($page>1):?><a href="?page=<?php echo $page-1;?>&q=<?php echo urlencode($search);?>" class="admin-btn">← Préc.</a><?php endif;?>
  <span style="font-size:.8rem;color:var(--gray400);padding:.5rem">Page <?php echo $page;?>/<?php echo ceil($total/$per);?></span>
  <?php if($page<ceil($total/$per)):?><a href="?page=<?php echo $page+1;?>&q=<?php echo urlencode($search);?>" class="admin-btn">Suiv. →</a><?php endif;?>
</div>
<?php endif;?>
