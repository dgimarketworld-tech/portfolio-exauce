...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../fraude/index.php" class="nav-link active"><span class="nav-icon">🚨</span> Fraude</a>
...
<?php
$title='Fraude & Alertes';
$seuil = max(100, (int)($_GET['seuil'] ?? 5000));
$jours  = max(1,   (int)($_GET['jours']  ?? 7));
$suspicious=DB::all("SELECT t.*,c.numero,u.first_name,u.last_name,u.email FROM transactions t JOIN comptes c ON t.compte_id=c.id JOIN users u ON c.user_id=u.id WHERE t.montant > :s AND t.type IN('virement_out','retrait') AND t.cree_le >= DATE_SUB(NOW(), INTERVAL :j DAY) ORDER BY t.montant DESC LIMIT 50",['s'=>$seuil,'j'=>$jours]);
$failed_logins=DB::all("SELECT email,ip_address,COUNT(*) as tries,MAX(attempted_at) as last_try FROM login_attempts WHERE success=0 AND attempted_at >= DATE_SUB(NOW(), INTERVAL 24 HOUR) GROUP BY email,ip_address HAVING tries >= 3 ORDER BY tries DESC LIMIT 20");
?>
<form method="GET" style="display:flex;gap:.75rem;align-items:center;margin-bottom:1.25rem;background:white;padding:.85rem 1.25rem;border-radius:12px;border:1px solid #E2E8F0">
  <span style="font-size:.8rem;font-weight:600;color:#64748B">Filtres fraude :</span>
  <label style="font-size:.8rem;color:#475569">Seuil (€) <input type="number" name="seuil" value="<?php echo $seuil;?>" min="100" step="100" style="width:90px;padding:.3rem .5rem;border:1px solid #CBD5E1;border-radius:6px;font-size:.8rem;margin-left:.3rem"></label>
  <label style="font-size:.8rem;color:#475569">Période (jours) <input type="number" name="jours" value="<?php echo $jours;?>" min="1" max="90" style="width:60px;padding:.3rem .5rem;border:1px solid #CBD5E1;border-radius:6px;font-size:.8rem;margin-left:.3rem"></label>
  <button type="submit" style="padding:.35rem .9rem;background:#3B82F6;color:white;border:none;border-radius:6px;font-size:.8rem;cursor:pointer;font-weight:600">Appliquer</button>
</form>
<div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem">
  <div class="admin-card">
    <div class="admin-card-header"><div class="admin-card-title">⚠️ Transactions suspectes (><?php echo number_format($seuil,0,',',' ');?>€, <?php echo $jours;?>j)</div></div>
    <table class="admin-table"><thead><tr><th>Client</th><th>Montant</th><th>Type</th><th>Date</th></tr></thead>
    <tbody>
    <?php foreach($suspicious as $tx): ?>
    <tr style="background:rgba(220,38,38,.04)">
      <td><?php echo htmlspecialchars(($tx['first_name']??'').' '.($tx['last_name']??''),ENT_QUOTES,'UTF-8');?></td>
      <td style="font-weight:700;color:var(--red,#dc2626)"><?php echo format_money($tx['montant']);?></td>
      <td><?php echo str_replace('_',' ',$tx['type']);?></td>
      <td style="font-size:.75rem;color:var(--gray400)"><?php echo format_datetime($tx['cree_le']);?></td>
    </tr>
    <?php endforeach; ?>
    <?php if(empty($suspicious)): ?><tr><td colspan="4" style="text-align:center;padding:1.5rem;color:var(--gray400)">✓ Aucune transaction suspecte</td></tr><?php endif; ?>
    </tbody></table>
  </div>
  <div class="admin-card">
    <div class="admin-card-header"><div class="admin-card-title">🔒 Tentatives connexion échouées (24h)</div></div>
    <table class="admin-table"><thead><tr><th>Email</th><th>IP</th><th>Tentatives</th><th>Dernière</th></tr></thead>
    <tbody>
    <?php foreach($failed_logins as $fl): ?>
    <tr style="background:<?php echo $fl['tries']>=5?'rgba(220,38,38,.06)':'';?>">
      <td style="font-size:.8rem"><?php echo htmlspecialchars($fl['email']??'—',ENT_QUOTES,'UTF-8');?></td>
      <td style="font-family:monospace;font-size:.75rem"><?php echo htmlspecialchars($fl['ip_address'],ENT_QUOTES,'UTF-8');?></td>
      <td><span class="badge badge-<?php echo $fl['tries']>=5?'red':'gold';?>"><?php echo $fl['tries'];?>x</span></td>
      <td style="font-size:.75rem;color:var(--gray400)"><?php echo time_ago($fl['last_try']);?></td>
    </tr>
    <?php endforeach; ?>
    <?php if(empty($failed_logins)): ?><tr><td colspan="4" style="text-align:center;padding:1.5rem;color:var(--gray400)">✓ Aucune tentative suspecte</td></tr><?php endif; ?>
    </tbody></table>
  </div>
</div>
