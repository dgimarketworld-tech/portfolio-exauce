...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../comptes/index.php" class="nav-link active"><span class="nav-icon">🏦</span> Comptes</a>
...
$search=trim($_GET['q']??'');
$where='1=1'; $params=[];
if($search){$where.=" AND (c.numero LIKE :q OR u.first_name LIKE :q OR u.last_name LIKE :q)";$params['q']="%$search%";}
$total=(int)DB::scalar("SELECT COUNT(*) FROM comptes c JOIN users u ON c.user_id=u.id WHERE $where",$params);
$params['l']=50; $params['o']=0;
$comptes=DB::all("SELECT c.*,u.first_name,u.last_name,u.email FROM comptes c JOIN users u ON c.user_id=u.id WHERE $where ORDER BY c.ouvert_le DESC LIMIT :l OFFSET :o",$params);
?>
<div style="margin-bottom:1.25rem"><form method="GET"><div class="admin-search"><span>🔍</span><input type="text" name="q" value="<?php echo htmlspecialchars($search,ENT_QUOTES,'UTF-8');?>" placeholder="N° compte, nom client…"/></div></form></div>
<div class="admin-card">
  <table class="admin-table"><thead><tr><th>Numéro</th><th>Client</th><th>Type</th><th>Devise</th><th style="text-align:right">Solde</th><th>Statut</th><th>Ouvert le</th></tr></thead>
  <tbody>
  <?php foreach($comptes as $c): ?>
  <tr>
    <td style="font-family:monospace;font-size:.75rem"><?php echo htmlspecialchars($c['numero'],ENT_QUOTES,'UTF-8');?></td>
    <td><b><?php echo htmlspecialchars(($c['first_name']??'').' '.($c['last_name']??''),ENT_QUOTES,'UTF-8');?></b><br><span style="font-size:.72rem;color:var(--gray400)"><?php echo htmlspecialchars($c['email'],ENT_QUOTES,'UTF-8');?></span></td>
    <td><span class="badge badge-blue"><?php echo ucfirst($c['type']);?></span></td>
    <td><?php echo htmlspecialchars($c['devise'],ENT_QUOTES,'UTF-8');?></td>
    <td style="text-align:right;font-weight:700"><?php echo format_money($c['solde'],$c['devise']);?></td>
    <td><span class="badge badge-<?php echo $c['statut']==='actif'?'green':($c['statut']==='gele'?'gold':'red');?>"><?php echo ucfirst($c['statut']);?></span></td>
    <td style="font-size:.75rem;color:var(--gray400)"><?php echo format_date($c['ouvert_le']);?></td>
  </tr>
  <?php endforeach; ?>
  <?php if(empty($comptes)): ?><tr><td colspan="7" style="text-align:center;padding:2rem;color:var(--gray400)">Aucun compte</td></tr><?php endif; ?>
  </tbody></table>
</div>
<div style="margin-top:.75rem;font-size:.8rem;color:var(--gray400)"><?php echo $total;?> compte(s) au total</div>
