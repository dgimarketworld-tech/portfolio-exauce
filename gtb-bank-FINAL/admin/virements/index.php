...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../virements/index.php" class="nav-link active"><span class="nav-icon">⇄</span> Virements</a>
...
<?php
$title='Virements';
// Action annuler virement
if($_SERVER['REQUEST_METHOD']==='POST' && Security::verifyCsrf($_POST['_csrf']??'')){
    $tid=(int)($_POST['tid']??0);
    $action=$_POST['action']??'';
    if($tid && $action==='cancel'){
        $tx=DB::one("SELECT * FROM transactions WHERE id=:id AND type='virement_out' AND statut NOT IN('annule','failed')",['id'=>$tid]);
        if($tx){
            DB::update("UPDATE transactions SET statut='annule' WHERE id=:id",['id'=>$tid]);
            // Rembourser le solde
            DB::update("UPDATE comptes SET solde=solde+:m WHERE id=:c",['m'=>$tx['montant'],'c'=>$tx['compte_id']]);
            notify($tx['user_id']??0,'Virement annulé','Votre virement de '.number_format($tx['montant'],2,',',' ').' € a été annulé par notre équipe.','warning');
        }
        header('Location: index.php'); exit;
    }
}
$txs=DB::all("SELECT t.*,c.numero,c.user_id,u.first_name,u.last_name FROM transactions t JOIN comptes c ON t.compte_id=c.id JOIN users u ON c.user_id=u.id WHERE t.type IN('virement_in','virement_out') ORDER BY t.cree_le DESC LIMIT 100");
$total_out=DB::scalar("SELECT SUM(montant) FROM transactions WHERE type='virement_out' AND DATE(cree_le)=CURDATE()")??0;
$total_in=DB::scalar("SELECT SUM(montant) FROM transactions WHERE type='virement_in' AND DATE(cree_le)=CURDATE()")??0;
$csrf=csrf_token();
?>
<div class="admin-kpi-grid" style="margin-bottom:1.5rem">
  <div class="kpi-card"><div class="kpi-icon" style="background:#FEF2F2">📤</div><div class="kpi-label">Virements émis aujourd'hui</div><div class="kpi-value" style="color:var(--red,#dc2626)"><?php echo format_money($total_out);?></div></div>
  <div class="kpi-card"><div class="kpi-icon" style="background:#F0FDF4">📥</div><div class="kpi-label">Virements reçus aujourd'hui</div><div class="kpi-value" style="color:var(--green,#16a34a)"><?php echo format_money($total_in);?></div></div>
</div>
<div class="admin-card">
  <table class="admin-table"><thead><tr><th>Référence</th><th>Client</th><th>Type</th><th>Motif</th><th style="text-align:right">Montant</th><th>Statut</th><th>Date</th><th>Action</th></tr></thead>
  <tbody>
  <?php foreach($txs as $tx): $out=$tx['type']==='virement_out'; $annulable=$out && $tx['statut']==='completed'; ?>
  <tr>
    <td style="font-family:monospace;font-size:.73rem"><?php echo htmlspecialchars($tx['reference'],ENT_QUOTES,'UTF-8');?></td>
    <td><a href="../utilisateurs/index.php?search=<?php echo urlencode(($tx['first_name']??'').' '.($tx['last_name']??''));?>" style="color:var(--accent)"><?php echo htmlspecialchars(($tx['first_name']??'').' '.($tx['last_name']??''),ENT_QUOTES,'UTF-8');?></a></td>
    <td><span class="badge badge-<?php echo $out?'blue':'green';?>"><?php echo $out?'Émis':'Reçu';?></span></td>
    <td style="font-size:.78rem;color:var(--gray600)"><?php echo htmlspecialchars($tx['description']??'—',ENT_QUOTES,'UTF-8');?></td>
    <td style="text-align:right;font-weight:700;color:<?php echo $out?'var(--red,#dc2626)':'var(--green,#16a34a)';?>"><?php echo ($out?'-':'+').format_money($tx['montant']);?></td>
    <td><span class="badge badge-<?php echo $tx['statut']==='annule'?'red':($tx['statut']==='completed'?'green':'gold');?>"><?php echo ucfirst($tx['statut']);?></span></td>
    <td style="font-size:.75rem;color:var(--gray400)"><?php echo format_datetime($tx['cree_le']);?></td>
    <td><?php if($annulable): ?><form method="POST" style="display:inline" onsubmit="return confirm('Annuler ce virement et rembourser le client ?')"><input type="hidden" name="_csrf" value="<?php echo $csrf;?>"><input type="hidden" name="action" value="cancel"><input type="hidden" name="tid" value="<?php echo $tx['id'];?>"><button type="submit" style="padding:.3rem .7rem;background:#FEF2F2;color:#dc2626;border:1px solid #FECACA;border-radius:6px;font-size:.72rem;cursor:pointer;font-weight:600">Annuler</button></form><?php else: echo '—'; endif; ?></td>
  </tr>
  <?php endforeach; ?>
  </tbody></table>
</div>
