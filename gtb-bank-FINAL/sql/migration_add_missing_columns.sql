...
ALTER TABLE users
  ...
  ADD COLUMN IF NOT EXISTS avatar_url          VARCHAR(255) NULL          AFTER two_fa_enabled,
  ADD COLUMN IF NOT EXISTS language            CHAR(5)      NOT NULL DEFAULT 'fr' AFTER avatar_url,
  ...
  ADD COLUMN IF NOT EXISTS access_blocked      TINYINT(1)   NOT NULL DEFAULT 0 AFTER failed_logins,
  ...
  ADD COLUMN IF NOT EXISTS pref_theme          VARCHAR(20)  NOT NULL DEFAULT 'light' AFTER transfer_stop_pct,
  ADD COLUMN IF NOT EXISTS pref_email_alerts   TINYINT(1)   NOT NULL DEFAULT 1 AFTER pref_theme,
...
-- Synchroniser first_name/last_name depuis prenom/nom pour les comptes existants
UPDATE users SET
...
-- Remplir iban depuis numero pour les comptes existants (numero = IBAN ou num de compte)
UPDATE comptes SET
  iban = COALESCE(iban, numero),
  bic  = COALESCE(bic,  'GTBKFRPPXXX'),
  cree_le = COALESCE(cree_le, ouvert_le)
WHERE iban IS NULL OR bic IS NULL;

-- ────────────────────────────────────────────────────────────────
--  TABLE admins — colonnes manquantes (si elles n'existent pas)
-- ────────────────────────────────────────────────────────────────
