-- ================================================================
--  GTB - Schéma de base de données
--  Importer dans phpMyAdmin OU : mysql -u root -p < schema.sql
-- ================================================================

CREATE DATABASE IF NOT EXISTS gtb CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE gtb;

-- Désactiver temporairement les contraintes pour permettre les DROP
...
-- ================================================================
--  USERS - Tous les utilisateurs (clients + admins)
--  Champs alignés sur ton formulaire signup.html
-- ================================================================
CREATE TABLE users (
...
-- ================================================================
--  COMPTES BANCAIRES (1 user peut avoir N comptes)
-- ================================================================
CREATE TABLE comptes (
    id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    user_id         INT UNSIGNED NOT NULL,
    numero          VARCHAR(34)  NOT NULL UNIQUE COMMENT 'IBAN ou numéro',
...
--  Indispensable en bancaire pour la conformité.
-- ================================================================
...
-- ================================================================
--  LOGIN ATTEMPTS - Pour le throttling anti-bruteforce
-- ================================================================
...
CREATE TABLE otp_codes (
    id         INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    user_id    INT UNSIGNED NOT NULL,
    code_hash  VARCHAR(255) NOT NULL COMMENT 'Le code OTP est aussi hashé',
...
-- ================================================================
--  Note : pas d'INSERT pour l'admin ici.
--  Lance bin/create-admin.php pour créer le compte admin
--  avec un hash Argon2id généré dynamiquement.
-- ================================================================

-- ================================================================
--  EXTENSION SCHEMA — virements, bénéficiaires, crédits,
--  support, notifications, KYC, sessions, admins
-- ================================================================
...
CREATE TABLE admins (
    ...
    created_at     DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    permissions    JSON         NULL COMMENT 'Droits spécifiques par module',
...
ALTER TABLE users
    ...
    ADD COLUMN IF NOT EXISTS civility        ENUM('M.','Mme') NULL AFTER email,
    ADD COLUMN IF NOT EXISTS client_number   VARCHAR(20) NULL AFTER id,
    ...
    ADD COLUMN IF NOT EXISTS two_fa_enabled  TINYINT(1) NOT NULL DEFAULT 0 AFTER kyc_status,
    ADD COLUMN IF NOT EXISTS avatar_url      VARCHAR(255) NULL AFTER kyc_status,
    ADD COLUMN IF NOT EXISTS language        CHAR(5) NOT NULL DEFAULT 'fr' AFTER two_fa_enabled,
    ...
    ADD COLUMN IF NOT EXISTS kyc_document_type  ENUM('cni_ue','passeport','permis','autre') NULL AFTER kyc_status,
    ADD COLUMN IF NOT EXISTS kyc_issuing_country CHAR(2) NULL AFTER kyc_document_type;
