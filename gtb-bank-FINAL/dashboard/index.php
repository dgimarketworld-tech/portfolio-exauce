...
.dash-tx-grid{display:grid;grid-template-columns:2fr 1fr;gap:1.25rem;margin-top:1.5rem}
...
@media (max-width: 768px) {
  ...
  .dash-tx-grid { grid-template-columns: 1fr; }
...
@media (max-width: 768px) {
  ...
  .dash-main { padding-bottom: calc(80px + env(safe-area-inset-bottom)); }
}
...
/* ── BOTTOM NAV MOBILE (style app) ──────────────────────── */
.mobile-bottom-nav{
  display:none;
  position:fixed;bottom:0;left:0;right:0;z-index:500;
  background:white;border-top:1px solid #E9ECEF;
  padding:.5rem 0 calc(.5rem + env(safe-area-inset-bottom));
  box-shadow:0 -4px 20px rgba(13,27,42,.1);
}
.mobile-bottom-nav-inner{display:flex;justify-content:space-around;align-items:center}
.mbn-item{
  display:flex;flex-direction:column;align-items:center;gap:.2rem;
  flex:1;padding:.4rem;cursor:pointer;
  font-family:'DM Sans',sans-serif;text-decoration:none;
  color:#6C757D;transition:color .2s;
}
.mbn-item.active{color:#0D1B2A}
.mbn-icon{font-size:1.25rem;line-height:1}
.mbn-label{font-size:.58rem;font-weight:600;letter-spacing:.02em}
.mbn-plus{
  width:44px;height:44px;border-radius:50%;
  background:linear-gradient(135deg,#0D1B2A,#091520);
  display:flex;align-items:center;justify-content:center;
...
<aside class="sidebar" id="sidebar">
  ...
  <div class="sidebar-user">
    ...
    <div class="sidebar-user-info">
      <div class="sidebar-user-name"><?php echo $fullname; ?></div>
      <div class="sidebar-user-plan"><?php echo $plan==='premium'?'✦ Premium':ucfirst($plan); ?></div>
      <div style="font-size:.58rem;color:rgba(255,255,255,.3);margin-top:3px;font-family:'JetBrains Mono',monospace;letter-spacing:.04em" title="Votre IBAN — cliquez pour copier" onclick="navigator.clipboard.writeText('<?php echo $iban_raw; ?>');this.textContent='✓ Copié!';setTimeout(()=>this.textContent='<?php echo $iban_mask; ?>',1500)" style="cursor:pointer"><?php echo $iban_mask; ?></div>
    </div>
  </div>
  <nav class="sidebar-nav">
    <div class="sidebar-section-label">Principal</div>
    <a href="index.php" class="sidebar-link active"><span class="sidebar-icon">⌂</span> Tableau de bord </a>
  ...
  <div class="sidebar-footer">
    <a href="../authentification/api/logout.php" class="sidebar-footer-link" onclick="return confirm('Se déconnecter ?')"><span>🔒</span> Déconnexion sécurisée</a>
    <a href="../dashboard/support/index.php" class="sidebar-footer-link"><span>❓</span> Aide &amp; Support</a>
  </div>
</aside>
<header class="topbar" id="topbar">
  <div class="topbar-left">
    <button class="sidebar-toggle" id="sidebarToggle" aria-label="Menu">☰</button>
    <div><div class="topbar-page-title"><?php echo $title??'Dashboard'; ?></div><div class="topbar-breadcrumb"><?php echo $breadcrumb??''; ?></div></div>
  </div>
  <div class="topbar-search"><span style="color:var(--gray400)">🔍</span><input type="text" placeholder="Rechercher…" id="globalSearch"/></div>
  <div class="topbar-right">
    <a href="notifications.php" class="topbar-btn" style="position:relative" title="Notifications">
      🔔<?php if($notif_count>0): ?><span class="topbar-notif-dot"></span><?php endif; ?>
    </a>
    <div class="topbar-user" onclick="location.href='profil/index.php'" style="cursor:pointer">
      <div class="topbar-user-av"><?php echo $initials; ?></div>
      <span class="topbar-user-name"><?php echo e($u['first_name']??''); ?></span>
      <span style="color:var(--gray400);font-size:.7rem">▾</span>
    </div>
  </div>
</header>
...
$breadcrumb = 'Bonjour, '.e($u['first_name']??'').' 👋';
...
$compte_id = $compte_principal['id'] ?? 0;

$transactions = DB::all("SELECT t.*, c.numero as compte_numero
    FROM transactions t JOIN comptes c ON t.compte_id=c.id
    WHERE c.user_id=:id ORDER BY t.cree_le DESC LIMIT 8",['id'=>Session::userId()]);

$cartes = DB::all("SELECT ca.*, co.numero FROM cartes ca JOIN comptes co ON ca.compte_id=co.id
    WHERE co.user_id=:id AND ca.statut='active' LIMIT 2",['id'=>Session::userId()]);

$credits = DB::all("SELECT * FROM credits WHERE user_id=:id AND statut='en_cours' LIMIT 3",['id'=>Session::userId()]);
?>

<div style="margin-bottom:1.5rem" class="anim-up">
  <h1 style="font-family:'Sora',sans-serif;font-weight:800;font-size:clamp(1.1rem,2.5vw,1.4rem);color:var(--bnp-dark)">Tableau de bord</h1>
  <p style="font-size:.82rem;color:var(--gray400);margin-top:.3rem"><?php echo date('l d F Y'); ?></p>
</div>
...
<div class="stats-grid anim-up d1">
  ...
  <div class="stat-card">
    <div class="stat-icon" style="background:<?php echo $c['type']==='courant'?'#EFF6FF':'#F0FDF4'; ?>"><?php echo $c['type']==='courant'?'🏦':'💰'; ?></div>
    <div class="stat-label"><?php echo ucfirst($c['type']); ?></div>
    <div class="stat-value"><?php echo format_money($c['solde'],$c['devise']); ?></div>
    <div class="stat-delta up"><?php echo e($c['numero']); ?></div>
  </div>
  <?php endforeach; ?>
  ...
  <div class="stat-card">
    ...
    <div class="stat-delta">Aucun compte actif</div>
  </div>
  <?php endif; ?>
</div>

<!-- Transactions récentes -->
<div class="anim-up d2 dash-tx-grid">
  <div class="card card-pad">
    <div class="card-header">
      <div><div class="card-title">Dernières opérations</div></div>
      <a href="../dashboard/transactions.php" class="btn btn-ghost btn-sm">Voir tout</a>
    </div>
    <?php if(empty($transactions)): ?>
    <div style="text-align:center;padding:2rem;color:var(--gray400)">Aucune transaction</div>
    <?php else: ?>
    <?php foreach($transactions as $tx): ?>
    <div class="row-item">
      <div class="row-icon" style="background:<?php echo strpos($tx['type'],'in')!==false||$tx['type']==='depot'?'#F0FDF4':'#FFF8E1'; ?>">
        <?php echo strpos($tx['type'],'in')!==false||$tx['type']==='depot'?'💰':'💸'; ?>
      </div>
      <div class="row-info">
        <div class="row-title"><?php echo e($tx['description']??ucfirst(str_replace('_',' ',$tx['type']))); ?></div>
        <div class="row-sub"><?php echo format_datetime($tx['cree_le']); ?> · Réf. <?php echo e($tx['reference']); ?></div>
      </div>
      <div style="text-align:right;flex-shrink:0">
        <div style="font-family:'Sora',sans-serif;font-weight:700;font-size:.88rem;color:<?php echo strpos($tx['type'],'in')!==false||$tx['type']==='depot'?'var(--green)':'var(--red)'; ?>">
          <?php echo (strpos($tx['type'],'in')!==false||$tx['type']==='depot'?'+':'-').format_money($tx['montant']); ?>
        </div>
        <div style="font-size:.7rem;color:var(--gray400);margin-top:.15rem"><?php echo format_money($tx['solde_apres']); ?></div>
      </div>
    </div>
    <?php endforeach; ?>
    <?php endif; ?>
  ...
  <!-- Carte bancaire -->
  <div>
    <?php if(!empty($cartes)): $carte = $cartes[0]; ?>
    <div class="card card-pad" style="background:linear-gradient(135deg,var(--bnp-deeper),#1a2e40);margin-bottom:1rem">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1.5rem">
        <div style="font-family:'Sora',sans-serif;font-size:.72rem;color:rgba(255,255,255,.5);text-transform:uppercase;letter-spacing:.08em"><?php echo ucfirst($carte['type']); ?></div>
        <div style="font-size:1.1rem;color:rgba(255,255,255,.7)"><?php echo strtoupper($carte['reseau']); ?></div>
      </div>
      <div style="font-family:'JetBrains Mono',monospace;font-size:.9rem;color:white;letter-spacing:.18em;margin-bottom:1.5rem"><?php echo e($carte['numero_masque']); ?></div>
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div style="font-size:.7rem;color:rgba(255,255,255,.4)">Expire <span style="color:rgba(255,255,255,.7)"><?php echo date('m/y',strtotime($carte['expire_le'])); ?></span></div>
        <span class="badge badge-green" style="font-size:.62rem">● Active</span>
      </div>
    </div>
    <?php else: ?>
    <div class="card card-pad" style="text-align:center;padding:2rem">
      <div style="font-size:.85rem;color:var(--gray400);margin-bottom:1rem">Aucune carte active</div>
      <a href="../dashboard/cartes/demande.php" class="btn btn-primary btn-sm">Demander une carte</a>
    </div>
    <?php endif; ?>
    ...
    <div class="card card-pad">
      <div class="card-header"><div class="card-title">Crédits en cours</div></div>
      <?php foreach($credits as $cr): ?>
      <div class="row-item">
        <div class="row-icon" style="background:#FFF8E1">📋</div>
        <div class="row-info">
          <div class="row-title"><?php echo ucfirst($cr['type']); ?></div>
          <div class="row-sub"><?php echo format_money($cr['mensualite']); ?>/mois</div>
        </div>
        <div style="text-align:right">
          <div style="font-size:.78rem;font-weight:600;color:var(--bnp-dark)"><?php echo format_money($cr['solde_restant']); ?></div>
          <div style="font-size:.68rem;color:var(--gray400)">restant</div>
        </div>
      </div>
      <?php endforeach; ?>
    </div>
...
<nav class="mobile-bottom-nav">
  <div class="mobile-bottom-nav-inner">
    <a href="index.php" class="mbn-item active">
      <span class="mbn-icon">⌂</span>
      <span class="mbn-label">Accueil</span>
    </a>
    <a href="virement/index.php" class="mbn-item">
      <span class="mbn-icon">⇄</span>
      <span class="mbn-label">Virements</span>
    </a>
    <a href="cartes/index.php" class="mbn-item" style="flex:0 0 auto">
      <span class="mbn-plus">+</span>
    </a>
    <a href="transactions.php" class="mbn-item">
      <span class="mbn-icon">📊</span>
      <span class="mbn-label">Transactions</span>
    </a>
    <a href="profil/index.php" class="mbn-item">
      <span class="mbn-icon">👤</span>
