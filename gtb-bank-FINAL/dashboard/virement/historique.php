...
<?php
$title='Historique virements'; $breadcrumb='Virements · Historique';
...
elseif($type_f==='recus') $where.=" AND t.type='virement_in'";
...
?>
<div class="sec-head"><div><h1 class="pg-title">Historique des virements</h1></div>
<div style="display:flex;gap:.5rem"><a href="index.php" class="btn btn-primary btn-sm">+ Nouveau</a><button class="btn btn-outline btn-sm" onclick="window.print()">📥 Export</button></div></div>
<div class="card card-pad" style="margin-bottom:1.25rem">
  <div class="chips" id="typeChips">
    <button class="chip <?php echo $type_f==='all'?'active':''; ?>" data-filter="all" onclick="location.href='?type=all'">Tous</button>
    <button class="chip <?php echo $type_f==='emis'?'active':''; ?>" data-filter="emis" onclick="location.href='?type=emis'">Émis</button>
    <button class="chip <?php echo $type_f==='recus'?'active':''; ?>" data-filter="recus" onclick="location.href='?type=recus'">Reçus</button>
  </div>
</div>
<div class="card"><div class="tbl-wrap">
<table class="tbl"><thead><tr><th>Date</th><th>Description</th><th>Référence</th><th style="text-align:right">Montant</th><th>Statut</th></tr></thead>
<tbody>
<?php foreach($txs as $tx): $in=$tx['type']==='virement_in'; ?>
<tr>
  <td><?php echo format_datetime($tx['cree_le']); ?></td>
  <td><?php echo e($tx['description']??'Virement'); ?></td>
...
<?php endforeach; ?>
<?php if(empty($txs)): ?><tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--gray400)">Aucun virement</td></tr><?php endif; ?>
</tbody></table></div></div>
</main></div>
