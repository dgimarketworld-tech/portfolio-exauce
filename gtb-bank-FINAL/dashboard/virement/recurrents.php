...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../virement/index.php" class="sidebar-link active"><span class="sidebar-icon">⇄</span> Virements </a>
  ...
  <div class="sidebar-footer">
    <a href="../../authentification/api/logout.php" class="sidebar-footer-link" onclick="return confirm('Se déconnecter ?')"><span>🔒</span> Déconnexion</a>
  </div>
</aside>
<header class="topbar" id="topbar">
  <div class="topbar-left"><button class="sidebar-toggle" id="sidebarToggle" aria-label="Menu">☰</button><div><div class="topbar-page-title"><?php echo $title??''; ?></div><div class="topbar-breadcrumb"><?php echo $breadcrumb??''; ?></div></div></div>
  <div class="topbar-search"><span style="color:var(--gray400)">🔍</span><input type="text" placeholder="Rechercher…"/></div>
  <div class="topbar-right">
    <a href="../notifications.php" class="topbar-btn">🔔<?php if($notif_count>0):?><span class="topbar-notif-dot"></span><?php endif;?></a>
    <div class="topbar-user" onclick="location.href='../profil/index.php'" style="cursor:pointer"><div class="topbar-user-av"><?php echo $initials; ?></div><span class="topbar-user-name"><?php echo e($u['first_name']??''); ?></span></div>
  </div>
</header>
<main class="dash-main">
<?php
$title='Virements récurrents'; $breadcrumb='Virements · Récurrents';
$error=''; $success='';
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrf($_POST['_csrf']??'')){
    ...
    if($action==='delete'){
        ...
        $iban=trim($_POST['iban']??''); $nom=trim($_POST['nom_dest']??'');
        $montant=(float)($_POST['montant']??0); $freq=$_POST['frequence']??'mensuel';
        $jour=(int)($_POST['jour']??1); $motif=trim($_POST['motif']??'');
...
$csrf=csrf_token();
?>
<div class="sec-head"><div><h1 class="pg-title">Virements récurrents</h1></div><button class="btn btn-primary btn-sm" onclick="openModal('modalNewRecur')">+ Nouveau récurrent</button></div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;margin-bottom:1.25rem">
  <div class="stat-card"><div class="stat-icon" style="background:#F0FDF4">🔄</div><div class="stat-label">Actifs</div><div class="stat-value"><?php echo count($recurrents); ?></div></div>
  <div class="stat-card"><div class="stat-icon" style="background:#FFF8E1">💸</div><div class="stat-label">Total mensuel</div><div class="stat-value"><?php echo format_money($total_mensuel); ?></div></div>
</div>
<div class="card card-pad">
<?php foreach($recurrents as $r): ?>
<div style="display:grid;grid-template-columns:1fr auto auto;align-items:center;gap:1rem;padding:.9rem 0;border-bottom:1px solid var(--gray50)">
  <div>
    <div style="font-weight:700;font-size:.88rem;color:var(--bnp-dark)"><?php echo e($r['nom_dest']); ?></div>
    <div style="font-size:.72rem;color:var(--gray400);margin-top:.15rem"><?php echo e($r['iban_dest']); ?> · <?php echo ucfirst($r['frequence']); ?> · <?php echo $r['jour_execution']; ?> du mois</div>
    <?php if($r['motif']): ?><div style="font-size:.72rem;color:var(--gray600);margin-top:.1rem"><?php echo e($r['motif']); ?></div><?php endif; ?>
  </div>
  <div style="text-align:right">
    <div style="font-family:'Sora',sans-serif;font-weight:700;font-size:.95rem"><?php echo format_money($r['montant']); ?></div>
    <div style="font-size:.68rem;color:var(--gray400)">Prochain : <?php echo format_date($r['prochain_le']); ?></div>
  </div>
  <form method="POST" onsubmit="return confirm('Annuler ce virement récurrent ?')">
    <input type="hidden" name="_csrf" value="<?php echo e($csrf); ?>"/>
    <input type="hidden" name="action" value="delete"/>
    <input type="hidden" name="vr_id" value="<?php echo $r['id']; ?>"/>
    <button type="submit" class="btn btn-sm btn-danger">🗑</button>
  </form>
</div>
<?php endforeach; ?>
<?php if(empty($recurrents)): ?><div style="text-align:center;padding:2rem;color:var(--gray400)">Aucun virement récurrent</div><?php endif; ?>
</div>
<!-- Modal -->
<div class="modal-overlay" id="modalNewRecur">
  <div class="modal-box">
    <div class="modal-head"><span class="modal-title">Nouveau virement récurrent</span><button class="modal-close" onclick="closeModal('modalNewRecur')">✕</button></div>
    <form method="POST"><div class="modal-body">
      <input type="hidden" name="_csrf" value="<?php echo e($csrf); ?>"/>
      <input type="hidden" name="action" value="add"/>
      <div class="form-group"><label class="form-label">Compte source</label><select name="compte_id" class="form-select" required><?php foreach($comptes as $c): ?><option value="<?php echo $c['id']; ?>"><?php echo ucfirst($c['type']).' — '.e(substr($c['numero'],-8)); ?></option><?php endforeach; ?></select></div>
      <div class="form-group"><label class="form-label">Bénéficiaire</label><input class="form-input" name="nom_dest" placeholder="Nom" required/></div>
      <div class="form-group"><label class="form-label">IBAN</label><input class="form-input" name="iban" placeholder="FR76…" required/></div>
      <div class="form-row">
        <div class="form-group"><label class="form-label">Montant (€)</label><input class="form-input" name="montant" type="number" step="0.01" required/></div>
        <div class="form-group"><label class="form-label">Fréquence</label><select name="frequence" class="form-select"><option value="mensuel">Mensuel</option><option value="hebdomadaire">Hebdomadaire</option><option value="trimestriel">Trimestriel</option></select></div>
      </div>
      <div class="form-row">
        <div class="form-group"><label class="form-label">Jour du mois</label><input class="form-input" name="jour" type="number" min="1" max="28" value="1"/></div>
        <div class="form-group"><label class="form-label">Motif</label><input class="form-input" name="motif" placeholder="Loyer, Abonnement…"/></div>
      </div>
    </div>
    <div class="modal-foot"><button type="button" class="btn btn-outline" onclick="closeModal('modalNewRecur')">Annuler</button><button type="submit" class="btn btn-primary">Créer</button></div>
    </form>
  </div>
</div>
</main></div>
