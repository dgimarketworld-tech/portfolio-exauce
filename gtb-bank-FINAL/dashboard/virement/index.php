...
<?php
$title='Nouveau virement'; $breadcrumb='Virements · Nouveau';
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    $csrf=$_POST['_csrf']??''; if(!Security::verifyCsrf($csrf)){$error='Token CSRF invalide.';}
    else{
        ...
        // Validations
        if(!$from_id||!$iban||!$nom||$montant<=0){$error='Tous les champs sont obligatoires.';}
        else{
            $compte=DB::one("SELECT * FROM comptes WHERE id=:id AND user_id=:uid AND statut='actif'",['id'=>$from_id,'uid'=>Session::userId()]);
            if(!$compte){$error='Compte source invalide.';}
            elseif($compte['solde']<$montant){$error='Solde insuffisant.';}
            else{
                $frais=$type_vir==='instant'?TRANSFER_FEE_INSTANT:TRANSFER_FEE_SEPA;
                $total=$montant+$frais;
                if($compte['solde']<$total){$error='Solde insuffisant (frais inclus).';}
                else{
...
$csrf=csrf_token();
?>
<div class="sec-head"><div><h1 class="pg-title">Nouveau virement</h1><p class="pg-sub">Virement SEPA ou instantané</p></div>
<a href="historique.php" class="btn btn-outline btn-sm">Historique</a></div>

<?php if($error): ?><div style="background:rgba(229,55,58,.08);border:1px solid rgba(229,55,58,.2);border-radius:var(--r-md);padding:.85rem 1rem;margin-bottom:1.25rem;font-size:.82rem;color:var(--red)">❌ <?php echo e($error); ?></div><?php endif; ?>
<?php if($success): ?><div style="background:rgba(0,198,122,.08);border:1px solid rgba(0,198,122,.2);border-radius:var(--r-md);padding:.85rem 1rem;margin-bottom:1.25rem;font-size:.82rem;color:var(--green)">✅ <?php echo e($success); ?></div><?php endif; ?>

<div style="display:grid;grid-template-columns:1fr 280px;gap:1.5rem;align-items:start">
<div class="card card-pad">
<form method="POST">
  <input type="hidden" name="_csrf" value="<?php echo e($csrf); ?>"/>
  <div class="form-group"><label class="form-label">Compte source</label>
  <select name="from_compte" class="form-select" required onchange="updateSolde(this)">
    <option value="">Sélectionner un compte</option>
    <?php foreach($comptes as $c): ?><option value="<?php echo $c['id']; ?>" data-solde="<?php echo $c['solde']; ?>"><?php echo ucfirst($c['type']).' — '.format_money($c['solde'],$c['devise']).' — '.e(substr($c['numero'],-8)); ?></option><?php endforeach; ?>
  </select>
  <div id="solde_disp" class="form-hint"></div></div>
  <div class="form-group"><label class="form-label">Bénéficiaire</label>
  <select name="_ben" class="form-select" onchange="fillBen(this)">
    <option value="">Saisir manuellement ou choisir…</option>
    <?php foreach($beneficiaires as $b): ?><option value="<?php echo e($b['iban']); ?>" data-nom="<?php echo e($b['nom']); ?>"><?php echo e($b['nom']).' — '.e($b['iban']); ?></option><?php endforeach; ?>
  </select></div>
  <div class="form-group"><label class="form-label">Nom du bénéficiaire</label><input class="form-input" name="nom_dest" id="nom_dest" placeholder="Prénom Nom ou Raison sociale" required/></div>
  <div class="form-group"><label class="form-label">IBAN</label><input class="form-input" name="iban" id="iban_dest" placeholder="FR76 …" required/></div>
  <div class="form-row">
    <div class="form-group"><label class="form-label">Montant (€)</label><input class="form-input" name="montant" type="number" step="0.01" min="0.01" placeholder="0,00" required/></div>
    <div class="form-group"><label class="form-label">Type</label>
    <select name="type_vir" class="form-select">
      <option value="sepa">SEPA standard (gratuit, J+1)</option>
      <option value="instant">Instantané (<?php echo format_money(TRANSFER_FEE_INSTANT); ?>, &lt;10s)</option>
    </select></div>
  </div>
  <div class="form-group"><label class="form-label">Motif (optionnel)</label><input class="form-input" name="motif" placeholder="Ex: Loyer, Remboursement…" maxlength="140"/></div>
  <button type="submit" class="btn btn-primary" style="width:100%">Exécuter le virement →</button>
</form>
</div>
<div class="card card-pad">
  <div class="card-title" style="margin-bottom:1rem">Informations</div>
  <div style="display:flex;flex-direction:column;gap:.65rem;font-size:.82rem;color:var(--gray600)">
    <div>🕐 <b>SEPA standard</b> : J+1 ouvré, gratuit</div>
    <div>⚡ <b>Instantané</b> : &lt;10 secondes, <?php echo format_money(TRANSFER_FEE_INSTANT); ?></div>
    <div>📊 <b>Plafond journalier</b> : <?php echo format_money(TRANSFER_LIMIT_DAILY); ?></div>
  </div>
  <div style="margin-top:1.5rem"><a href="beneficiaires.php" class="btn btn-outline btn-sm" style="width:100%;justify-content:center">Gérer mes bénéficiaires</a></div>
</div>
</div>
</main></div>
...
function fillBen(sel){const o=sel.options[sel.selectedIndex];if(o.value){document.getElementById('iban_dest').value=o.value;document.getElementById('nom_dest').value=o.dataset.nom||'';}}
</script>
