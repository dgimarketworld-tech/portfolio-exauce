...
<?php
$title='Mes bénéficiaires'; $breadcrumb='Virements · Bénéficiaires';
$error=''; $success='';
if($_SERVER['REQUEST_METHOD']==='POST'){
    if(!Security::verifyCsrf($_POST['_csrf']??'')){$error='Token invalide.';}
    else{
        $action=$_POST['action']??'';
        if($action==='add'){
            $nom=trim($_POST['nom']??''); $iban=strtoupper(preg_replace('/\s+/','',$_POST['iban']??''));
            $bic=strtoupper(trim($_POST['bic']??'')); $intitule=trim($_POST['intitule']??'');
...
$csrf=csrf_token();
?>
<div class="sec-head"><div><h1 class="pg-title">Mes bénéficiaires</h1><p class="pg-sub"><?php echo count($bens); ?> enregistré(s)</p></div>
<button class="btn btn-primary btn-sm" onclick="openModal('modalAddBen')">+ Ajouter</button></div>
...
<div class="card card-pad">
  <div style="width:48px;height:48px;border-radius:50%;background:linear-gradient(135deg,var(--bnp-green),var(--bnp-emerald));display:flex;align-items:center;justify-content:center;font-family:'Sora',sans-serif;font-weight:800;color:#fff;font-size:.85rem;margin-bottom:.85rem"><?php echo strtoupper(substr($b['nom'],0,2)); ?></div>
  <div style="font-family:'Sora',sans-serif;font-weight:700;font-size:.95rem;color:var(--bnp-dark);margin-bottom:.2rem"><?php echo e($b['nom']); ?></div>
  <div style="font-family:monospace;font-size:.78rem;color:var(--gray400);margin-bottom:.15rem"><?php echo e($b['iban']); ?></div>
  <?php if($b['intitule']): ?><div style="font-size:.75rem;color:var(--gray600)"><?php echo e($b['intitule']); ?></div><?php endif; ?>
  <div style="display:flex;gap:.4rem;margin-top:1rem">
    <a href="index.php" class="btn btn-primary btn-sm">Virer</a>
    <form method="POST" style="display:inline" onsubmit="return confirm('Supprimer ?')">
      <input type="hidden" name="_csrf" value="<?php echo e($csrf); ?>"/>
      <input type="hidden" name="action" value="delete"/>
      <input type="hidden" name="ben_id" value="<?php echo $b['id']; ?>"/>
...
<?php endforeach; ?>
<?php if(empty($bens)): ?>
<div class="card card-pad" style="grid-column:1/-1;text-align:center;padding:3rem">
  <div style="font-size:2rem;margin-bottom:.75rem">👥</div>
  <div style="font-family:'Sora',sans-serif;font-weight:700">Aucun bénéficiaire</div>
  <button class="btn btn-primary btn-sm" style="margin-top:1rem" onclick="openModal('modalAddBen')">+ Ajouter le premier</button>
</div>
<?php endif; ?>
</div>
<!-- Modal ajout -->
<div class="modal-overlay" id="modalAddBen">
  <div class="modal-box">
    <div class="modal-head"><span class="modal-title">Ajouter un bénéficiaire</span><button class="modal-close" onclick="closeModal('modalAddBen')">✕</button></div>
    <form method="POST"><div class="modal-body">
      <input type="hidden" name="_csrf" value="<?php echo e($csrf); ?>"/>
      <input type="hidden" name="action" value="add"/>
      <div class="info-box" style="margin-bottom:1rem">🔒 Un SMS de confirmation vous sera envoyé.</div>
      <div class="form-group"><label class="form-label">Nom du bénéficiaire</label><input class="form-input" name="nom" placeholder="Prénom Nom" required/></div>
      <div class="form-group"><label class="form-label">IBAN</label><input class="form-input" name="iban" placeholder="FR76…" required/></div>
      <div class="form-group"><label class="form-label">BIC (optionnel)</label><input class="form-input" name="bic" placeholder="GTBKFRPPXXX"/></div>
      <div class="form-group"><label class="form-label">Intitulé (optionnel)</label><input class="form-input" name="intitule" placeholder="Ex: Loyer, Fournisseur…"/></div>
    </div>
    <div class="modal-foot"><button type="button" class="btn btn-outline" onclick="closeModal('modalAddBen')">Annuler</button><button type="submit" class="btn btn-primary">Ajouter</button></div>
    </form>
  </div>
</div>
</main></div>
