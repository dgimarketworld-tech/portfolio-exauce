...
<?php
$title='Détail ticket'; $breadcrumb='Support · Ticket';
$id=(int)($_GET['id']??0);
...
// Mark conseiller messages as read
DB::update("UPDATE support_messages SET lu=1 WHERE ticket_id=:id AND auteur_type='conseiller' AND lu=0",['id'=>$id]);
$title='Ticket #'.e($ticket['reference']);
$csrf=csrf_token();
?>
<div class="sec-head">
  <div><a href="index.php" style="font-size:.8rem;color:var(--gray400);text-decoration:none">← Retour au support</a>
  <h1 class="pg-title" style="margin-top:.4rem"><?php echo e($ticket['reference']);?> — <?php echo e($ticket['sujet']);?></h1>
  <div style="display:flex;align-items:center;gap:.6rem;margin-top:.35rem">
    <span class="badge badge-<?php echo ['ouvert'=>'blue','en_cours'=>'gold','resolu'=>'green','ferme'=>'gray'][$ticket['statut']]??'gray';?>"><?php echo ucfirst(str_replace('_',' ',$ticket['statut']));?></span>
    <span style="font-size:.78rem;color:var(--gray400)"><?php echo format_date($ticket['cree_le']);?></span>
  </div></div>
</div>
<div style="display:grid;grid-template-columns:1fr 280px;gap:1.25rem;align-items:start">
  <div class="card card-pad">
    <div class="card-title" style="margin-bottom:1.2rem">Conversation</div>
    <div id="convo" style="display:flex;flex-direction:column;gap:1rem;max-height:450px;overflow-y:auto;margin-bottom:1.5rem;padding-right:.25rem">
    <?php foreach($messages as $msg): $is_client=$msg['auteur_type']==='client'; ?>
    <div style="display:flex;flex-direction:column;align-items:<?php echo $is_client?'flex-end':'flex-start';?>">
      <div style="max-width:85%;border-radius:<?php echo $is_client?'var(--r-lg) var(--r-lg) 4px var(--r-lg)':'var(--r-lg) var(--r-lg) var(--r-lg) 4px';?>;padding:.85rem 1.1rem;font-size:.83rem;line-height:1.6;background:<?php echo $is_client?'var(--bnp-dark)':'var(--gray50)';?>;color:<?php echo $is_client?'#fff':'var(--gray800)';?>;border:<?php echo $is_client?'none':'1px solid var(--gray100)';?>"><?php echo nl2br(e($msg['message']));?></div>
      <div style="font-size:.68rem;color:var(--gray400);margin-top:.3rem"><?php echo $is_client?'Vous':e(($ticket['c_fn']??'Conseiller').' '.($ticket['c_ln']??''));?> · <?php echo time_ago($msg['cree_le']);?></div>
    </div>
    <?php endforeach;?>
    </div>
    <?php if($ticket['statut']!=='ferme'):?>
    <div style="border-top:1px solid var(--gray100);padding-top:1.2rem">
      <form method="POST">
        <input type="hidden" name="_csrf" value="<?php echo e($csrf);?>"/>
        <div class="form-group"><label class="form-label">Votre réponse</label><textarea class="form-textarea" name="message" placeholder="Écrivez votre message…" rows="3" required></textarea></div>
        <div style="display:flex;gap:.5rem"><button type="button" class="btn btn-outline btn-sm" onclick="showToast('Pièce jointe ajoutée','info')">📎 Joindre</button><button type="submit" class="btn btn-primary" style="margin-left:auto">Envoyer →</button></div>
      </form>
    </div>
    <?php endif;?>
  </div>
  <div style="display:flex;flex-direction:column;gap:1rem">
    <div class="card card-pad">
      <div class="card-title" style="margin-bottom:1rem">Détails</div>
      <div style="display:flex;flex-direction:column;gap:.5rem;font-size:.82rem">
        <div style="display:flex;justify-content:space-between;padding:.5rem 0;border-bottom:1px solid var(--gray50)"><span style="color:var(--gray400)">Référence</span><span style="font-family:monospace;font-weight:700">#<?php echo e($ticket['reference']);?></span></div>
        <div style="display:flex;justify-content:space-between;padding:.5rem 0;border-bottom:1px solid var(--gray50)"><span style="color:var(--gray400)">Catégorie</span><span style="font-weight:600"><?php echo ucfirst($ticket['categorie']);?></span></div>
        <div style="display:flex;justify-content:space-between;padding:.5rem 0;border-bottom:1px solid var(--gray50)"><span style="color:var(--gray400)">Priorité</span><span class="badge badge-<?php echo $ticket['priorite']==='urgente'?'red':'blue';?>"><?php echo ucfirst($ticket['priorite']);?></span></div>
        <div style="display:flex;justify-content:space-between;padding:.5rem 0"><span style="color:var(--gray400)">Conseiller</span><span style="font-weight:600"><?php echo $ticket['c_fn']?e($ticket['c_fn'].' '.$ticket['c_ln']):'Non assigné';?></span></div>
      </div>
    </div>
  </div>
</div>
<script>document.getElementById('convo')?.scrollTo(0,9999);</script>
