...
<?php
$title='Support client'; $breadcrumb='Aide &amp; Support';
...
$open=array_filter($tickets,fn($t)=>in_array($t['statut'],['ouvert','en_cours']));
?>
<div class="sec-head"><div><h1 class="pg-title">Support client</h1><p class="pg-sub">Nous répondons sous 24h ouvrées</p></div>
<a href="nouveau.php" class="btn btn-primary btn-sm">+ Nouvelle demande</a></div>
<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin-bottom:1.5rem">
  <div class="sup-contact" onclick="showToast('0800 GTB BANK','info','📞')" style="background:#fff;border:1.5px solid var(--gray100);border-radius:var(--r-xl);padding:1.4rem;cursor:pointer;text-align:center;transition:all .3s">
    <div style="font-size:1.5rem;margin-bottom:.5rem">📞</div><div style="font-weight:700;font-size:.85rem">Téléphone</div><div style="font-size:.72rem;color:var(--gray400)">0800 GTB BANK</div>
  </div>
  <div class="sup-contact" onclick="showToast('Chat ouvert — attente ~2 min','success','💬')" style="background:#fff;border:1.5px solid var(--gray100);border-radius:var(--r-xl);padding:1.4rem;cursor:pointer;text-align:center;transition:all .3s">
    <div style="font-size:1.5rem;margin-bottom:.5rem">💬</div><div style="font-weight:700;font-size:.85rem">Chat en direct</div><div style="font-size:.72rem;color:var(--green)">● En ligne</div>
  </div>
  <a href="nouveau.php" style="background:#fff;border:1.5px solid var(--gray100);border-radius:var(--r-xl);padding:1.4rem;cursor:pointer;text-align:center;text-decoration:none;display:block;transition:all .3s">
    <div style="font-size:1.5rem;margin-bottom:.5rem">✉️</div><div style="font-weight:700;font-size:.85rem;color:var(--bnp-dark)">Créer un ticket</div><div style="font-size:.72rem;color:var(--gray400)">Réponse &lt;24h</div>
  </a>
</div>
<div class="card card-pad">
  <div class="card-header"><div class="card-title">Mes demandes (<?php echo count($tickets);?>)</div></div>
  <?php foreach($tickets as $t): ?>
  <a href="detail.php?id=<?php echo $t['id'];?>" style="display:block;padding:1rem;border-radius:var(--r-md);border:1.5px solid var(--gray100);margin-bottom:.65rem;text-decoration:none;transition:all .2s;color:inherit" onmouseover="this.style.borderColor='var(--bnp-emerald)'" onmouseout="this.style.borderColor='var(--gray100)'">
    <div style="display:flex;justify-content:space-between;margin-bottom:.35rem">
      <b style="font-size:.88rem">#<?php echo e($t['reference']);?> — <?php echo e($t['sujet']);?></b>
      <span class="badge badge-<?php echo ['ouvert'=>'blue','en_cours'=>'gold','resolu'=>'green','ferme'=>'gray'][$t['statut']]??'gray';?>"><?php echo ucfirst(str_replace('_',' ',$t['statut']));?></span>
    </div>
    <div style="font-size:.72rem;color:var(--gray400)"><?php echo format_date($t['cree_le']);?> · <?php echo ucfirst($t['categorie']);?> · <?php echo $t['c_fn']?e($t['c_fn'].' '.$t['c_ln']):'Non assigné';?></div>
  </a>
  <?php endforeach;?>
  <?php if(empty($tickets)):?><div style="text-align:center;padding:2.5rem;color:var(--gray400)">Aucun ticket — <a href="nouveau.php" style="color:var(--bnp-emerald)">Créer une demande</a></div><?php endif;?>
</div>
