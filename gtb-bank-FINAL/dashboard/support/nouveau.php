...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../support/index.php" class="sidebar-link active"><span class="sidebar-icon">❓</span> Support </a>
...
<header class="topbar" id="topbar">
  <div class="topbar-left"><button class="sidebar-toggle" id="sidebarToggle">☰</button><div><div class="topbar-page-title"><?php echo $title??'';?></div><div class="topbar-breadcrumb"><?php echo $breadcrumb??'';?></div></div></div>
  <div class="topbar-search"><span style="color:var(--gray400)">🔍</span><input type="text" placeholder="Rechercher…"/></div>
  <div class="topbar-right"><a href="../notifications.php" class="topbar-btn">🔔<?php if($notif_count>0):?><span class="topbar-notif-dot"></span><?php endif;?></a><div class="topbar-user" onclick="location.href='../profil/index.php'" style="cursor:pointer"><div class="topbar-user-av"><?php echo $initials;?></div><span class="topbar-user-name"><?php echo e($u['first_name']??'');?></span></div></div>
</header><main class="dash-main">
<?php
$title='Nouveau ticket'; $breadcrumb='Support · Nouvelle demande';
$error=''; $success=''; $ticket_ref='';
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrf($_POST['_csrf']??'')){
    $cat=trim($_POST['categorie']??''); $sujet=trim($_POST['sujet']??'');
    $desc=trim($_POST['description']??''); $prio=$_POST['priorite']??'normale';
    if(!$cat||!$sujet||!$desc){$error='Tous les champs sont obligatoires.';}
    else{
        ...
        send_notification_email(Session::userId(),'Votre ticket support — GTB Bank',"Votre ticket <strong>#$ref</strong> a été créé avec succès. Notre équipe support vous répondra sous <strong>24h ouvrées</strong>.");
        ...
        $html_adm="<div style='font-family:Arial,sans-serif;max-width:520px;margin:auto;padding:32px;border:1px solid #e5e7eb;border-radius:8px'><h2 style='color:#1a3c5e'>Nouveau ticket support</h2><table style='width:100%;border-collapse:collapse;color:#374151'><tr><td style='padding:6px 0;font-weight:600'>Client</td><td>{$u['prenom']} {$u['nom']} ({$u['email']})</td></tr><tr><td style='padding:6px 0;font-weight:600'>Référence</td><td>$ref</td></tr><tr><td style='padding:6px 0;font-weight:600'>Sujet</td><td>$sujet</td></tr><tr><td style='padding:6px 0;font-weight:600'>Catégorie</td><td>$cat</td></tr><tr><td style='padding:6px 0;font-weight:600'>Priorité</td><td>$prio</td></tr><tr><td style='padding:6px 0;font-weight:600'>Date</td><td>".date('d/m/Y H:i')."</td></tr></table><a href='".GTB_BASE_URL."/admin/support/index.php' style='display:inline-block;margin:20px 0;padding:12px 24px;background:#1a3c5e;color:#fff;text-decoration:none;border-radius:6px;font-weight:600'>Voir le ticket</a><p style='color:#9ca3af;font-size:12px'>Global Trust Bank — Notification automatique</p></div>";
        send_email(MAIL_SUPPORT,'Admin GTB',"Ticket support #$ref — {$u['prenom']} {$u['nom']}",$html_adm);
        $ticket_ref=$ref; $success="Votre ticket <strong>#$ref</strong> a été créé. Réponse sous 24h ouvrées.";
    }
}
$csrf=csrf_token();
?>
<div class="sec-head"><div><a href="index.php" style="font-size:.8rem;color:var(--gray400);text-decoration:none">← Retour au support</a><h1 class="pg-title" style="margin-top:.4rem">Nouvelle demande</h1></div></div>
<?php if($success):?>
<div style="background:rgba(0,198,122,.08);border:1px solid rgba(0,198,122,.2);border-radius:var(--r-xl);padding:2rem;text-align:center">
  <div style="font-size:2rem;margin-bottom:.75rem">✅</div>
  <div style="font-family:'Sora',sans-serif;font-weight:800;font-size:1.1rem;color:var(--bnp-dark);margin-bottom:.5rem">Demande envoyée !</div>
  <div style="font-size:.85rem;color:var(--gray600);margin-bottom:1.5rem"><?php echo $success;?></div>
  <div style="display:flex;gap:.5rem;justify-content:center"><a href="index.php" class="btn btn-primary">Voir mes tickets</a><a href="../index.php" class="btn btn-outline">Tableau de bord</a></div>
</div>
<?php else:?>
...
<div class="card card-pad">
  <form method="POST">
    ...
    <div class="form-group"><label class="form-label">Sujet</label><input class="form-input" name="sujet" placeholder="Résumez en une phrase" maxlength="180" required/></div>
    <div class="form-group"><label class="form-label">Description détaillée</label><textarea class="form-textarea" name="description" placeholder="Date, montant, circonstances précises…" rows="6" required></textarea></div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">Priorité</label><select name="priorite" class="form-select"><option value="normale">Normale</option><option value="urgente">Urgente</option><option value="basse">Basse</option></select></div>
      <div class="form-group"><label class="form-label">Canal de réponse préféré</label><select name="canal" class="form-select"><option>Email</option><option>SMS</option></select></div>
    </div>
...
<div style="display:flex;flex-direction:column;gap:1rem">
  <div class="card card-pad">
    <div style="font-size:.68rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--gray400);margin-bottom:.85rem">⚡ Actions rapides</div>
    <button class="btn btn-outline btn-sm" style="width:100%;justify-content:flex-start;margin-bottom:.4rem" onclick="showToast('Appel en cours…','info','📞')">📞 Appeler le support</button>
    <button class="btn btn-outline btn-sm" style="width:100%;justify-content:flex-start" onclick="showToast('Chat ouvert','success','💬')">💬 Chat en direct</button>
  </div>
  <div class="card card-pad info-box">
    <div style="font-weight:700;margin-bottom:.5rem">💡 Avant de créer</div>
    <div style="line-height:1.7">• Pour bloquer votre carte : Cartes → Bloquer<br>• Pour contester : Transactions → Contester<br>• Urgence : appelez le 0800 GTB BANK</div>
  </div>
</div>
</div>
<?php endif;?>
