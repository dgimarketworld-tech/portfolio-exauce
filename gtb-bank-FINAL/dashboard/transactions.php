...
<aside class="sidebar" id="sidebar">
  ...
  <div class="sidebar-footer">
    ...
    <a href="../dashboard/support/index.php" class="sidebar-footer-link"><span>❓</span> Support</a>
  </div>
</aside>
...
<?php
$title='Transactions'; $breadcrumb='Tableau de bord · Transactions';
$per_page=20; $page=max(1,(int)($_GET['page']??1));
$type_filter=$_GET['type']??'';
$where='c.user_id=:uid'; $params=['uid'=>Session::userId()];
if($type_filter&&in_array($type_filter,['depot','retrait','virement_in','virement_out'])){$where.=' AND t.type=:type';$params['type']=$type_filter;}
...
$params['limit']=$per_page;$params['offset']=$pg['offset'];
$txs=DB::all("SELECT t.*,c.numero FROM transactions t JOIN comptes c ON t.compte_id=c.id WHERE $where ORDER BY t.cree_le DESC LIMIT :limit OFFSET :offset",$params);
?>
<div class="sec-head"><div><h1 class="pg-title">Transactions</h1><p class="pg-sub"><?php echo $total; ?> opération(s) au total</p></div>
<button class="btn btn-outline btn-sm" onclick="window.print()">📥 Exporter</button></div>
...
<form method="GET" style="display:flex;gap:.5rem;flex-wrap:wrap;align-items:center">
  <select name="type" class="form-select" style="width:180px" onchange="this.form.submit()">
    ...
    <option value="depot" <?php echo $type_filter==='depot'?'selected':''; ?>>Dépôts</option>
    <option value="retrait" <?php echo $type_filter==='retrait'?'selected':''; ?>>Retraits</option>
    <option value="virement_in" <?php echo $type_filter==='virement_in'?'selected':''; ?>>Virements reçus</option>
    <option value="virement_out" <?php echo $type_filter==='virement_out'?'selected':''; ?>>Virements émis</option>
  </select>
</form>
</div>
<div class="card"><div class="tbl-wrap">
<table class="tbl"><thead><tr><th>Date</th><th>Description</th><th>Référence</th><th>Compte</th><th style="text-align:right">Montant</th><th>Statut</th></tr></thead>
<tbody>
<?php foreach($txs as $tx): $in=strpos($tx['type'],'in')!==false||$tx['type']==='depot'; ?>
<tr>
  <td><?php echo format_datetime($tx['cree_le']); ?></td>
  <td><b><?php echo e($tx['description']??ucfirst(str_replace('_',' ',$tx['type']))); ?></b></td>
  <td style="font-family:monospace;font-size:.75rem;color:var(--gray400)"><?php echo e($tx['reference']); ?></td>
  <td style="font-size:.78rem;color:var(--gray400)"><?php echo e($tx['numero']); ?></td>
  <td style="text-align:right;font-family:'Sora',sans-serif;font-weight:700;color:<?php echo $in?'var(--green)':'var(--red)'; ?>">
    <?php echo ($in?'+':'-').format_money($tx['montant']); ?>
  </td>
  <td><span class="badge badge-<?php echo $tx['statut']==='terminee'?'green':($tx['statut']==='en_cours'?'blue':'red'); ?>"><?php echo ucfirst($tx['statut']); ?></span></td>
</tr>
<?php endforeach; ?>
<?php if(empty($txs)): ?><tr><td colspan="6" style="text-align:center;padding:2rem;color:var(--gray400)">Aucune transaction</td></tr><?php endif; ?>
</tbody></table></div></div>
<?php if($pg['total_pages']>1): ?>
<div style="display:flex;justify-content:center;gap:.5rem;margin-top:1rem">
  <?php if($pg['has_prev']): ?><a href="?page=<?php echo $pg['current']-1; ?>&type=<?php echo $type_filter; ?>" class="btn btn-outline btn-sm">← Préc.</a><?php endif; ?>
  <span style="font-size:.8rem;color:var(--gray400);padding:.5rem">Page <?php echo $pg['current']; ?>/<?php echo $pg['total_pages']; ?></span>
  <?php if($pg['has_next']): ?><a href="?page=<?php echo $pg['current']+1; ?>&type=<?php echo $type_filter; ?>" class="btn btn-outline btn-sm">Suiv. →</a><?php endif; ?>
</div>
<?php endif; ?>
</main>
</div>
