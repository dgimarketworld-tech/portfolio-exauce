...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../parametres/index.php" class="sidebar-link active"><span class="sidebar-icon">⚙</span> Paramètres </a>
...
<header class="topbar" id="topbar"><div class="topbar-left"><button class="sidebar-toggle" id="sidebarToggle">☰</button><div><div class="topbar-page-title"><?php echo $title??''?></div><div class="topbar-breadcrumb"><?php echo $breadcrumb??''?></div></div></div><div class="topbar-right"><a href="../notifications.php" class="topbar-btn">🔔<?php if($notif_count>0):?><span class="topbar-notif-dot"></span><?php endif;?></a><div class="topbar-user" style="cursor:pointer" onclick="location.href='../profil/index.php'"><div class="topbar-user-av"><?php echo $initials;?></div><span class="topbar-user-name"><?php echo e($u['first_name']??'')?></span></div></div></header>
<main class="dash-main">
<?php
$title='Paramètres'; $breadcrumb='Paramètres du compte';
...
if(!$settings){$settings=['plafond_virement'=>5000,'plafond_paiement'=>2000,'notif_email'=>1,'notif_sms'=>1,'notif_push'=>0,'langue'=>'fr','devise'=>'EUR','theme'=>'dark'];}
$success='';
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrf($_POST['_csrf']??'')){
    $tab=$_POST['_tab']??'notifs';
    if($tab==='paiements'){
        $plafond_virement=(float)($_POST['plafond_mensuel']??5000);
        $plafond_paiement=(float)($_POST['plafond_retrait']??2000);
        DB::pdo()->prepare("INSERT INTO user_settings (user_id,plafond_virement,plafond_paiement) VALUES (:uid,:pv,:pp) ON DUPLICATE KEY UPDATE plafond_virement=:pv2,plafond_paiement=:pp2")->execute(['uid'=>$uid,'pv'=>$plafond_virement,'pp'=>$plafond_paiement,'pv2'=>$plafond_virement,'pp2'=>$plafond_paiement]);
        $settings['plafond_virement']=$plafond_virement;$settings['plafond_paiement']=$plafond_paiement;
        ...
        DB::pdo()->prepare("INSERT INTO user_settings (user_id,notif_email,notif_sms,notif_push) VALUES (:uid,:ne,:ns,:np) ON DUPLICATE KEY UPDATE notif_email=:ne2,notif_sms=:ns2,notif_push=:np2")->execute(['uid'=>$uid,'ne'=>$notif_email,'ns'=>$notif_sms,'np'=>$notif_push,'ne2'=>$notif_email,'ns2'=>$notif_sms,'np2'=>$notif_push]);
        $settings['notif_email']=$notif_email;$settings['notif_sms']=$notif_sms;$settings['notif_push']=$notif_push;
...
$csrf=csrf_token();
?>
<div class="sec-head"><div><h1 class="pg-title">Paramètres</h1><p class="pg-sub">Gérez votre compte et vos préférences</p></div></div>
<?php if($success):?><div style="background:rgba(0,198,122,.08);border:1px solid rgba(0,198,122,.2);border-radius:var(--r-md);padding:.85rem;margin-bottom:1.25rem;font-size:.82rem;color:var(--green)"><?php echo e($success);?></div><?php endif;?>
<div style="display:grid;grid-template-columns:200px 1fr;gap:1.25rem;align-items:start" id="pLayout">
  <div class="card card-pad" style="padding:.5rem">
    <nav id="pNav">
      <button class="pnav-btn active" data-tab="profil">👤 Profil</button>
      <button class="pnav-btn" data-tab="securite">🔒 Sécurité</button>
      <button class="pnav-btn" data-tab="notifs">🔔 Notifications</button>
      <button class="pnav-btn" data-tab="paiements">💳 Paiements</button>
      <button class="pnav-btn" data-tab="confidentialite">🛡️ Confidentialité</button>
    </nav>
  </div>
  <div class="card card-pad">
    <div class="ppanel active" id="pp-profil">
      <div class="card-header"><div class="card-title">Informations personnelles</div></div>
      <p style="font-size:.82rem;color:var(--gray600);margin-bottom:1rem">Modifiez vos informations depuis votre profil.</p>
      <a href="../profil/index.php" class="btn btn-primary btn-sm">Aller à Mon Profil →</a>
    </div>
    <div class="ppanel" id="pp-securite">
      <div class="card-header"><div class="card-title">Sécurité</div></div>
      <div class="p-row"><div><div class="p-row-lbl">Mot de passe</div></div><a href="../profil/mot-de-passe.php" class="btn btn-outline btn-sm">Modifier</a></div>
      <div class="p-row"><div><div class="p-row-lbl">Authentification 2FA</div><div class="p-row-sub">SMS à chaque connexion</div></div><div class="toggle on" data-label="2FA"></div></div>
      <div class="p-row"><div><div class="p-row-lbl">Alertes de connexion</div></div><div class="toggle on" data-label="Alertes connexion"></div></div>
      <div style="margin-top:1rem"><a href="../profil/securite.php" class="btn btn-outline btn-sm">Gérer les sessions →</a></div>
    </div>
    <div class="ppanel" id="pp-notifs">
      <div class="card-header"><div class="card-title">Notifications</div></div>
      <div class="p-row"><div><div class="p-row-lbl">Chaque débit ou crédit</div></div><div class="toggle on" data-label="Notif transactions"></div></div>
      <div class="p-row"><div><div class="p-row-lbl">Virement reçu</div></div><div class="toggle on" data-label="Virement reçu"></div></div>
      <div class="p-row"><div><div class="p-row-lbl">Notifications push</div></div><div class="toggle on" data-label="Push"></div></div>
      <div class="p-row"><div><div class="p-row-lbl">Email</div></div><div class="toggle on" data-label="Email"></div></div>
      <div class="p-row"><div><div class="p-row-lbl">SMS</div></div><div class="toggle" data-label="SMS"></div></div>
    </div>
    <div class="ppanel" id="pp-paiements">
      <form method="POST"><input type="hidden" name="_csrf" value="<?php echo e($csrf);?>"/><input type="hidden" name="_tab" value="paiements"/>
      <div class="card-header"><div class="card-title">Paiements</div></div>
      <div class="p-row"><div><div class="p-row-lbl">Paiements sans contact (NFC)</div></div><div class="toggle on" data-label="NFC"></div></div>
      <div class="p-row"><div><div class="p-row-lbl">Paiements en ligne</div></div><div class="toggle on" data-label="Paiements en ligne"></div></div>
      <div class="p-row"><div><div class="p-row-lbl">Paiements à l'étranger</div></div><div class="toggle on" data-label="Étranger"></div></div>
      <div style="margin-top:1rem">
        <div class="form-group"><label class="form-label">Plafond mensuel carte (€)</label><input class="form-input" name="plafond_mensuel" type="number" value="<?php echo (float)($settings['plafond_virement']??5000);?>"/></div>
        <div class="form-group"><label class="form-label">Plafond paiement (€)</label><input class="form-input" name="plafond_retrait" type="number" value="<?php echo (float)($settings['plafond_paiement']??2000);?>"/></div>
        <button type="submit" class="btn btn-primary">Enregistrer</button>
      </div>
      </form>
    </div>
    <div class="ppanel" id="pp-confidentialite">
      <div class="card-header"><div class="card-title">Confidentialité &amp; Données</div></div>
      <div class="p-row"><div><div class="p-row-lbl">Partage données d'analyse</div></div><div class="toggle" data-label="Analyse"></div></div>
      <div class="p-row"><div><div class="p-row-lbl">Cookies personnalisation</div></div><div class="toggle on" data-label="Cookies"></div></div>
      <div class="p-row"><div><div class="p-row-lbl">Communications marketing</div></div><div class="toggle" data-label="Marketing"></div></div>
      <div style="margin-top:1.2rem;display:flex;gap:.65rem;flex-wrap:wrap">
        <button class="btn btn-outline btn-sm" onclick="showToast('Export RGPD en cours…','info')">📥 Télécharger mes données</button>
      </div>
    </div>
  </div>
</div>
...
.p-row{display:flex;align-items:center;justify-content:space-between;padding:.82rem 0;border-bottom:1px solid var(--gray50);gap:1rem}
.p-row:last-of-type{border-bottom:none}.p-row-lbl{font-size:.85rem;font-weight:600;color:var(--bnp-dark)}.p-row-sub{font-size:.72rem;color:var(--gray400);margin-top:.12rem}
@media(max-width:768px){#pLayout{grid-template-columns:1fr}#pNav{display:flex;overflow-x:auto;gap:.3rem;padding-bottom:.4rem}.pnav-btn{white-space:nowrap;flex-shrink:0}}
</style>
<script>
document.querySelectorAll('.pnav-btn').forEach(btn=>{btn.addEventListener('click',()=>{
  ...
  btn.classList.add('active');document.getElementById('pp-'+btn.dataset.tab)?.classList.add('active');
});});
</script>
