...
<?php
$title='Préférences'; $breadcrumb='Profil · Préférences';
$uid=Session::userId();
try{DB::pdo()->exec("ALTER TABLE users ADD COLUMN IF NOT EXISTS pref_theme VARCHAR(20) DEFAULT 'dark'");}catch(\Exception $e){}
...
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrf($_POST['_csrf']??'')){
    ...
    $pref_theme=$_POST['pref_theme']??'dark';
    $pref_email=isset($_POST['pref_email_alerts'])?1:0;
    $pref_sms=isset($_POST['pref_sms_alerts'])?1:0;
    DB::pdo()->prepare("UPDATE users SET language=:l,pref_theme=:t,pref_email_alerts=:e,pref_sms_alerts=:s WHERE id=:id")->execute(['l'=>$langue,'t'=>$pref_theme,'e'=>$pref_email,'s'=>$pref_sms,'id'=>$uid]);
    $u=DB::one("SELECT * FROM users WHERE id=:id",['id'=>$uid])??$u;
    $success='Préférences enregistrées.';
}
$csrf=csrf_token();
?>
<div class="sec-head"><div><a href="index.php" style="font-size:.8rem;color:var(--gray400);text-decoration:none">← Retour au profil</a><h1 class="pg-title" style="margin-top:.4rem">Préférences</h1></div></div>
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem">
  <div class="card card-pad">
    <div class="card-title" style="margin-bottom:1.2rem">🌍 Langue &amp; Région</div>
      <div class="form-group"><label class="form-label">Langue</label>
        <select name="langue" class="form-select">
          <option value="fr" <?php echo ($u['language']??'fr')==='fr'?'selected':'';?>>Français</option>
          <option value="en" <?php echo ($u['language']??'')==='en'?'selected':'';?>>English</option>
        </select></div>
      <div class="form-group"><label class="form-label">Thème</label>
        <select name="pref_theme" class="form-select">
          <option value="dark" <?php echo ($u['pref_theme']??'dark')==='dark'?'selected':'';?>>Sombre</option>
          <option value="light" <?php echo ($u['pref_theme']??'')==='light'?'selected':'';?>>Clair</option>
        </select></div>
  </div>
  <div class="card card-pad">
    <div class="card-title" style="margin-bottom:1.2rem">🔔 Alertes</div>
    <div style="display:flex;justify-content:space-between;align-items:center;padding:.8rem 0;border-bottom:1px solid var(--gray50)"><label style="font-size:.85rem;font-weight:600;color:var(--bnp-dark)">Alertes par email</label><input type="checkbox" name="pref_email_alerts" value="1" <?php echo ($u['pref_email_alerts']??1)?'checked':'';?> style="width:20px;height:20px;cursor:pointer"/></div>
    <div style="display:flex;justify-content:space-between;align-items:center;padding:.8rem 0"><label style="font-size:.85rem;font-weight:600;color:var(--bnp-dark)">Alertes par SMS</label><input type="checkbox" name="pref_sms_alerts" value="1" <?php echo ($u['pref_sms_alerts']??1)?'checked':'';?> style="width:20px;height:20px;cursor:pointer"/></div>
  </div>
  <div class="card card-pad" style="grid-column:span 2;display:flex;justify-content:flex-end">
    <button type="submit" class="btn btn-primary">Enregistrer les préférences</button>
  </div>
  <div class="card card-pad">
    <div class="card-title" style="margin-bottom:1rem">🛡️ Confidentialité</div>
    <div style="display:flex;justify-content:space-between;align-items:center;padding:.8rem 0;border-bottom:1px solid var(--gray50)"><div style="font-size:.85rem;font-weight:600;color:var(--bnp-dark)">Partage données d'analyse</div><div class="toggle" data-label="Analyse"></div></div>
    <div style="display:flex;justify-content:space-between;align-items:center;padding:.8rem 0;border-bottom:1px solid var(--gray50)"><div style="font-size:.85rem;font-weight:600;color:var(--bnp-dark)">Cookies personnalisation</div><div class="toggle on" data-label="Cookies"></div></div>
    <div style="display:flex;justify-content:space-between;align-items:center;padding:.8rem 0"><div style="font-size:.85rem;font-weight:600;color:var(--bnp-dark)">Communications marketing</div><div class="toggle" data-label="Marketing"></div></div>
  </div>
  <div class="card card-pad">
    <div class="card-title" style="margin-bottom:1rem">⬇️ Mes données</div>
    <button class="btn btn-outline btn-sm" onclick="showToast('Export RGPD en cours de préparation…','info')">📥 Télécharger mes données (RGPD)</button>
    <div style="margin-top:1.2rem;padding-top:1.2rem;border-top:1px solid var(--gray100)">
      <div style="font-size:.82rem;color:var(--gray600);margin-bottom:.75rem">Zone dangereuse</div>
      <button class="btn btn-sm" style="background:rgba(229,55,58,.1);color:var(--red);border:none;cursor:pointer;border-radius:999px;padding:.5rem 1rem;font-weight:600" onclick="confirmDlg('Supprimer définitivement votre compte ? Action irréversible.',()=>showToast('Demande transmise au service client','info'),'danger')">🗑 Supprimer mon compte</button>
    </div>
  </div>
</div>
</form>
