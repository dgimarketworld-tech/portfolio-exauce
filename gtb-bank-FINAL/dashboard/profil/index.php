...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../profil/index.php" class="sidebar-link active"><span class="sidebar-icon">👤</span> Mon Profil </a>
...
<?php
$title='Mon Profil'; $breadcrumb='Tableau de bord · Mon Profil';
$success=''; $error='';
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrf($_POST['_csrf']??'')){
    $fn=trim($_POST['first_name']??''); $ln=trim($_POST['last_name']??'');
    $tel=trim($_POST['telephone']??''); $addr=trim($_POST['address']??'');
    if(!$fn||!$ln){$error='Prénom et nom obligatoires.';}
    else{
        ...
        $_SESSION['user']['first_name']=$fn; $_SESSION['user']['last_name']=$ln;
        $currentUser=array_merge($currentUser,['first_name'=>$fn,'last_name'=>$ln]);
        $u=$currentUser; $initials=strtoupper(substr($fn,0,1).substr($ln,0,1));
        $success='Profil mis à jour.';
    }
}
$csrf=csrf_token();
?>
<div class="sec-head"><div><h1 class="pg-title">Mon Profil</h1><p class="pg-sub">Informations personnelles et préférences</p></div>
<a href="../../dashboard/parametres/index.php" class="btn btn-outline btn-sm">⚙ Paramètres</a></div>
...
<div style="display:grid;grid-template-columns:280px 1fr;gap:1.5rem;align-items:start" id="profilLayout">
  <div style="display:flex;flex-direction:column;gap:1.25rem">
    <div class="card card-pad" style="text-align:center">
      <div style="width:72px;height:72px;border-radius:50%;background:linear-gradient(135deg,var(--bnp-green),var(--bnp-emerald));display:flex;align-items:center;justify-content:center;font-family:'Sora',sans-serif;font-weight:800;font-size:1.3rem;color:#fff;margin:0 auto 1rem"><?php echo $initials;?></div>
      <div style="font-family:'Sora',sans-serif;font-weight:700;font-size:1rem;color:var(--bnp-dark)"><?php echo e(($u['first_name']??'').' '.($u['last_name']??''));?></div>
      <div style="font-size:.78rem;color:var(--gray400);margin:.2rem 0 .75rem"><?php echo e($u['email']??'');?></div>
      <span class="badge badge-<?php echo ($u['plan']??'')==='premium'?'gold':'blue';?>"><?php echo ($u['plan']??'')==='premium'?'✦ Premium':'Standard';?></span>
      <div style="font-size:.72rem;color:var(--gray400);margin-top:.75rem"><?php echo e($u['client_number']??'');?></div>
    </div>
    <div class="card card-pad">
      <div style="font-size:.68rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--gray400);margin-bottom:.85rem">Accès rapides</div>
      <a href="mot-de-passe.php" style="display:flex;align-items:center;gap:.6rem;padding:.65rem 0;font-size:.83rem;color:var(--gray700);text-decoration:none;border-bottom:1px solid var(--gray50)">🔑 Mot de passe</a>
      <a href="securite.php" style="display:flex;align-items:center;gap:.6rem;padding:.65rem 0;font-size:.83rem;color:var(--gray700);text-decoration:none;border-bottom:1px solid var(--gray50)">🔒 Sécurité</a>
      <a href="preferences.php" style="display:flex;align-items:center;gap:.6rem;padding:.65rem 0;font-size:.83rem;color:var(--gray700);text-decoration:none">🎨 Préférences</a>
    </div>
  </div>
  <div class="card card-pad">
    ...
    <form method="POST">
      ...
      <div class="form-row">
        <div class="form-group"><label class="form-label">Prénom</label><input class="form-input" name="first_name" value="<?php echo e($u['first_name']??'');?>" required/></div>
        <div class="form-group"><label class="form-label">Nom</label><input class="form-input" name="last_name" value="<?php echo e($u['last_name']??'');?>" required/></div>
      </div>
      <div class="form-group"><label class="form-label">Email</label><input class="form-input" value="<?php echo e($u['email']??'');?>" disabled style="opacity:.6"/></div>
      <div class="form-group"><label class="form-label">Téléphone</label><input class="form-input" name="telephone" value="<?php echo e($u['telephone']??'');?>" placeholder="+33 6 …"/></div>
