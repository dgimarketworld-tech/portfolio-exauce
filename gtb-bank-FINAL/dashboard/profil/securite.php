...
<?php
$title='Sécurité'; $breadcrumb='Profil · Sécurité';
$uid=Session::userId();
...
$csrf=csrf_token();
?>
<div class="sec-head"><div><a href="index.php" style="font-size:.8rem;color:var(--gray400);text-decoration:none">← Retour au profil</a><h1 class="pg-title" style="margin-top:.4rem">Sécurité du compte</h1></div></div>
<?php if($msg==='session_closed'):?><div style="background:rgba(0,198,122,.08);border:1px solid rgba(0,198,122,.2);border-radius:var(--r-md);padding:.85rem;margin-bottom:1.25rem;font-size:.82rem;color:var(--green)">Session fermée.</div><?php endif;?>
<?php if($msg==='2fa_on'):?><div style="background:rgba(0,198,122,.08);border:1px solid rgba(0,198,122,.2);border-radius:var(--r-md);padding:.85rem;margin-bottom:1.25rem;font-size:.82rem;color:var(--green)">2FA activé.</div><?php elseif($msg==='2fa_off'):?><div style="background:rgba(229,55,58,.08);border:1px solid rgba(229,55,58,.2);border-radius:var(--r-md);padding:.85rem;margin-bottom:1.25rem;font-size:.82rem;color:var(--red)">2FA désactivé.</div><?php endif;?>
<div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem">
  <div class="card card-pad">
    <div class="card-header"><div><div class="card-title">Double authentification (2FA)</div></div><span class="badge <?php echo $twofa?'badge-green':'badge-red';?>"><?php echo $twofa?'Actif':'Inactif';?></span></div>
    <div style="display:flex;flex-direction:column;gap:.85rem">
      <div style="background:rgba(0,198,122,.06);border:1px solid rgba(0,198,122,.15);border-radius:var(--r-md);padding:.85rem;display:flex;align-items:center;gap:.85rem">
        <div style="font-size:1.3rem">📱</div><div><div style="font-weight:600;font-size:.85rem">SMS — <?php echo e(substr($u['telephone']??'••••••••',0,-4).'••••');?></div><div style="font-size:.72rem;color:var(--gray400)">Méthode principale</div></div><span class="badge badge-green" style="margin-left:auto">Actif</span>
      </div>
      <div style="background:var(--gray50);border:1px solid var(--gray100);border-radius:var(--r-md);padding:.85rem;display:flex;align-items:center;gap:.85rem">
        <div style="font-size:1.3rem">🔐</div><div style="flex:1"><div style="font-weight:600;font-size:.85rem">Authenticator App</div><div style="font-size:.72rem;color:var(--gray400)">Google Auth, Authy…</div></div>
        <button class="btn btn-outline btn-sm" onclick="showToast('QR Code généré — ouvrez votre app','info')">Activer</button>
      </div>
      <form method="POST" style="margin-top:1rem"><input type="hidden" name="_csrf" value="<?php echo e($csrf);?>"/><input type="hidden" name="toggle_2fa" value="1"/><button type="submit" class="btn <?php echo $twofa?'btn-danger':'btn-primary';?> btn-sm"><?php echo $twofa?'🔓 Désactiver la 2FA':'🔒 Activer la 2FA';?></button></form>
    </div>
  </div>
  <div class="card card-pad">
    <div class="card-header"><div class="card-title">Sessions actives</div><button class="btn btn-sm btn-danger" onclick="confirmDlg('Fermer toutes les autres sessions ?',()=>{showToast(\'Sessions fermées\',\'success\');},'warning')">Tout fermer</button></div>
    <div style="display:flex;align-items:center;gap:.85rem;padding:.85rem;background:rgba(0,198,122,.06);border:1px solid rgba(0,198,122,.15);border-radius:var(--r-md);margin-bottom:.5rem">
      <div style="font-size:1.3rem">💻</div><div style="flex:1"><div style="font-weight:600;font-size:.85rem">Session actuelle</div><div style="font-size:.72rem;color:var(--gray400)"><?php echo $_SERVER['HTTP_USER_AGENT']?substr($_SERVER['HTTP_USER_AGENT'],0,40).'…':'Navigateur inconnu';?></div></div><span class="badge badge-green">Actuelle</span>
    </div>
    <?php foreach($sessions as $s): if($s['id']===session_id()) continue; ?>
    <div style="display:flex;align-items:center;gap:.85rem;padding:.85rem;background:var(--gray50);border:1px solid var(--gray100);border-radius:var(--r-md);margin-bottom:.5rem">
      <div style="font-size:1.3rem">📱</div>
      <div style="flex:1"><div style="font-weight:600;font-size:.85rem"><?php echo e($s['device_name']??'Appareil inconnu');?></div><div style="font-size:.72rem;color:var(--gray400)"><?php echo time_ago($s['last_activity']);?> · <?php echo e($s['ip_address']??'IP inconnue');?></div></div>
      <form method="POST" style="display:inline"><input type="hidden" name="_csrf" value="<?php echo e($csrf);?>"/><input type="hidden" name="session_id" value="<?php echo e($s['id']);?>"/><button type="submit" class="btn btn-sm btn-danger">Fermer</button></form>
    </div>
    <?php endforeach;?>
  </div>
</div>
