...
<?php
$title='Mes Comptes'; $breadcrumb='Tableau de bord · Comptes';
$comptes=DB::all("SELECT * FROM comptes WHERE user_id=:id ORDER BY type",[  'id'=>Session::userId()]);
?>
<div class="sec-head"><div><h1 class="pg-title">Mes Comptes</h1><p class="pg-sub"><?php echo count($comptes); ?> compte(s) actif(s)</p></div>
<a href="../index.php" class="btn btn-outline btn-sm">← Retour</a></div>
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:1.25rem">
<?php foreach($comptes as $c): ?>
<div class="card card-pad" style="cursor:pointer" onclick="location.href='detail.php?id=<?php echo $c['id']; ?>'">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:1.25rem">
    <div>
      <div style="font-size:.68rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--gray400);margin-bottom:.3rem"><?php echo ucfirst($c['type']); ?></div>
      <div style="font-family:'Sora',sans-serif;font-weight:800;font-size:1.4rem;color:var(--bnp-dark)"><?php echo format_money($c['solde'],$c['devise']); ?></div>
    </div>
    <span class="badge badge-<?php echo $c['statut']==='actif'?'green':'red'; ?>"><?php echo ucfirst($c['statut']); ?></span>
  </div>
  <div style="font-family:monospace;font-size:.82rem;color:var(--gray600);margin-bottom:1rem;word-break:break-all"><?php echo e($c['numero']); ?></div>
  <div style="font-size:.72rem;color:var(--gray400)">Ouvert le <?php echo format_date($c['ouvert_le']); ?></div>
  <div style="margin-top:1rem;display:flex;gap:.5rem">
    <a href="detail.php?id=<?php echo $c['id']; ?>" class="btn btn-primary btn-sm" style="flex:1;justify-content:center">Voir détail</a>
    <a href="../virement/index.php" class="btn btn-outline btn-sm">Virer</a>
  </div>
</div>
<?php endforeach; ?>
<?php if(empty($comptes)): ?>
<div class="card card-pad" style="text-align:center;padding:3rem;grid-column:1/-1">
  <div style="font-size:2rem;margin-bottom:.75rem">🏦</div>
  <div style="font-family:'Sora',sans-serif;font-weight:700;color:var(--bnp-dark)">Aucun compte</div>
  <p style="font-size:.82rem;color:var(--gray400);margin-top:.5rem">Contactez votre conseiller pour ouvrir un compte.</p>
</div>
<?php endif; ?>
</div>
</main>
