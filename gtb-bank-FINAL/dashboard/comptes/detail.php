...
<aside class="sidebar" id="sidebar">
  ...
  <div class="sidebar-user">
    ...
    <div class="sidebar-user-info">
      ...
      <div class="sidebar-user-plan"><?php echo ($u['plan']??'')==='premium'?'✦ Premium':'Standard'; ?></div>
    </div>
  </div>
  <nav class="sidebar-nav">
    <div class="sidebar-section-label">Principal</div>
    <a href="../index.php" class="sidebar-link"><span class="sidebar-icon">⌂</span> Tableau de bord </a>
    <a href="../comptes/index.php" class="sidebar-link active"><span class="sidebar-icon">⊞</span> Mes Comptes </a>
  ...
  <div class="sidebar-footer">
    ...
    <a href="../support/index.php" class="sidebar-footer-link"><span>❓</span> Support</a>
  </div>
</aside>
<header class="topbar" id="topbar">
  <div class="topbar-left">
    <button class="sidebar-toggle" id="sidebarToggle" aria-label="Menu">☰</button>
    <div><div class="topbar-page-title"><?php echo $title??''; ?></div><div class="topbar-breadcrumb"><?php echo $breadcrumb??''; ?></div></div>
  </div>
  <div class="topbar-search"><span style="color:var(--gray400)">🔍</span><input type="text" placeholder="Rechercher…"/></div>
  <div class="topbar-right">
    <a href="../notifications.php" class="topbar-btn" style="position:relative">
...
<?php
$title='Détail du compte'; $breadcrumb='Comptes · Détail';
$id=(int)($_GET['id']??0);
...
?>
<div class="sec-head">
  <div><a href="index.php" style="font-size:.8rem;color:var(--gray400);text-decoration:none">← Retour aux comptes</a>
  <h1 class="pg-title" style="margin-top:.4rem"><?php echo ucfirst($compte['type']); ?> — <?php echo e(substr($compte['numero'],-8)); ?></h1></div>
  <span class="badge badge-<?php echo $compte['statut']==='actif'?'green':'red'; ?>"><?php echo ucfirst($compte['statut']); ?></span>
</div>
  ...
  <div>
    <div class="card card-pad" style="margin-bottom:1.25rem">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem">
        ...
        <div style="font-family:'Sora',sans-serif;font-weight:800;font-size:1.6rem;color:var(--bnp-dark)"><?php echo format_money($compte['solde'],$compte['devise']); ?></div>
      </div>
      <div style="font-family:monospace;font-size:.82rem;color:var(--gray600);word-break:break-all;padding:.75rem;background:var(--gray50);border-radius:var(--r-md)"><?php echo e($compte['numero']); ?></div>
    </div>
    <div class="card"><div class="tbl-wrap">
    <table class="tbl"><thead><tr><th>Date</th><th>Description</th><th>Référence</th><th style="text-align:right">Montant</th><th style="text-align:right">Solde après</th></tr></thead>
    <tbody>
    <?php foreach($txs as $tx): $in=strpos($tx['type'],'in')!==false||$tx['type']==='depot'; ?>
    <tr>
      <td><?php echo format_datetime($tx['cree_le']); ?></td>
      <td><?php echo e($tx['description']??ucfirst(str_replace('_',' ',$tx['type']))); ?></td>
      <td style="font-family:monospace;font-size:.75rem;color:var(--gray400)"><?php echo e($tx['reference']); ?></td>
      <td style="text-align:right;font-family:'Sora',sans-serif;font-weight:700;color:<?php echo $in?'var(--green)':'var(--red)'; ?>"><?php echo ($in?'+':'-').format_money($tx['montant']); ?></td>
      <td style="text-align:right;font-size:.8rem;color:var(--gray600)"><?php echo format_money($tx['solde_apres']); ?></td>
    </tr>
    <?php endforeach; ?>
    <?php if(empty($txs)): ?><tr><td colspan="5" style="text-align:center;padding:2rem;color:var(--gray400)">Aucune transaction</td></tr><?php endif; ?>
    </tbody></table></div></div>
  </div>
  <div style="display:flex;flex-direction:column;gap:1rem">
    <?php if($carte): ?>
    <div class="card card-pad" style="background:linear-gradient(135deg,var(--bnp-deeper),#1a2e40)">
      <div style="font-size:.7rem;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.08em;margin-bottom:1rem"><?php echo ucfirst($carte['type']); ?></div>
      <div style="font-family:monospace;font-size:.88rem;color:white;letter-spacing:.18em;margin-bottom:1.2rem"><?php echo e($carte['numero_masque']); ?></div>
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div style="font-size:.7rem;color:rgba(255,255,255,.4)">Expire <b style="color:rgba(255,255,255,.7)"><?php echo date('m/y',strtotime($carte['expire_le'])); ?></b></div>
    ...
    <div class="card card-pad">
      ...
      <a href="../virement/index.php" class="btn btn-primary btn-sm" style="width:100%;justify-content:center;margin-bottom:.4rem">⇄ Faire un virement</a>
      <a href="../releves.php" class="btn btn-outline btn-sm" style="width:100%;justify-content:center">📄 Relevé de compte</a>
    </div>
  </div>
</div>
</main>
