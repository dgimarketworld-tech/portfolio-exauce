...
thead th{background:#0D1B2A;color:#fff;padding:.5rem .75rem;text-align:left;font-size:.8rem}
...
.footer{margin-top:2rem;font-size:.75rem;color:#888;border-top:1px solid #ddd;padding-top:.75rem}
</style><link rel="stylesheet" href="mobile.css">
</head><body>
<h1>Global Trust Bank — Relevé de compte</h1>
<div class="meta">Compte : <?php echo htmlspecialchars(ucfirst($compte['type']).' — '.$compte['numero'],ENT_QUOTES,'UTF-8');?> &nbsp;|&nbsp; Titulaire : <?php echo $nom;?> &nbsp;|&nbsp; Solde actuel : <?php echo number_format((float)$compte['solde'],2,',',' ').' '.$compte['devise'];?><br>Édité le : <?php echo date('d/m/Y H:i');?> &nbsp;|&nbsp; 50 dernières opérations</div>
<table><thead><tr><th>Date</th><th>Description</th><th>Référence</th><th>Montant</th><th>Solde après</th></tr></thead><tbody>
<?php foreach($txs as $t):$pos=in_array($t['type'],['depot','virement_in','credit'],true);?>
<tr><td><?php echo date('d/m/Y H:i',strtotime($t['created_at']??'now'));?></td>
<td><?php echo htmlspecialchars($t['description']??'',ENT_QUOTES,'UTF-8');?></td>
<td style="font-family:monospace;font-size:.75rem"><?php echo htmlspecialchars($t['reference']??'',ENT_QUOTES,'UTF-8');?></td>
<td class="<?php echo $pos?'pos':'neg';?>"><?php echo ($pos?'+':'-').number_format(abs((float)$t['montant']),2,',',' ').' '.$compte['devise'];?></td>
<td><?php echo number_format((float)$t['solde_apres'],2,',',' ').' '.$compte['devise'];?></td></tr>
<?php endforeach;?>
</tbody></table>
...
<?php
$title='Mes relevés'; $breadcrumb='Tableau de bord · Relevés';
$comptes=DB::all("SELECT * FROM comptes WHERE user_id=:id",['id'=>Session::userId()]);
?>
<div class="sec-head"><div><h1 class="pg-title">Mes relevés</h1><p class="pg-sub">Relevés de compte mensuels</p></div></div>
...
<div class="card card-pad">
  <div class="card-header"><div class="card-title"><?php echo ucfirst($c['type']);?></div><div style="font-size:.72rem;color:var(--gray400)"><?php echo e(substr($c['numero'],-8));?></div></div>
  <?php foreach($mois as $m): list($y,$mo)=explode('-',$m);?>
  <div style="display:flex;justify-content:space-between;align-items:center;padding:.65rem 0;border-bottom:1px solid var(--gray50)">
    <div style="font-size:.83rem;font-weight:500"><?php $mnames=['01'=>'Janvier','02'=>'Février','03'=>'Mars','04'=>'Avril','05'=>'Mai','06'=>'Juin','07'=>'Juillet','08'=>'Août','09'=>'Septembre','10'=>'Octobre','11'=>'Novembre','12'=>'Décembre']; echo ($mnames[$mo]??$mo).' '.$y;?></div>
    <a href="?export=1&compte_id=<?php echo (int)$c['id'];?>" target="_blank" class="btn btn-outline btn-xs">📥 PDF</a>
  </div>
  <?php endforeach;?>
</div>
<?php endforeach;?>
