...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../avantage/index.php" class="sidebar-link active"><span class="sidebar-icon">✦</span> Avantages </a>
...
<?php
$title='Avantages GTB'; $breadcrumb='Mes Avantages';
$plan=$u['plan']??'standard';
?>
<div class="sec-head"><div><h1 class="pg-title">Mes Avantages</h1><p class="pg-sub">Profitez de votre statut <?php echo ucfirst($plan);?></p></div></div>
<div style="background:linear-gradient(135deg,var(--bnp-deeper),#1a2e40);border-radius:var(--r-xl);padding:1.75rem;color:#fff;margin-bottom:1.5rem;display:flex;align-items:center;gap:1.5rem;flex-wrap:wrap">
  <div style="font-size:2rem">✦</div>
  <div><div style="font-family:'Sora',sans-serif;font-weight:800;font-size:1.1rem;margin-bottom:.3rem">Statut <?php echo ucfirst($plan);?></div>
  <div style="font-size:.82rem;color:rgba(255,255,255,.6)"><?php echo $plan==='premium'?'Accès à tous les avantages exclusifs GTB Premier':'Passez Premium pour débloquer tous les avantages';?></div></div>
  <?php if($plan!=='premium'):?><a href="../../produits-bancaires/comptes/index.html" class="btn btn-sm" style="background:var(--bnp-emerald);color:white;border:none;margin-left:auto">Passer Premium</a><?php endif;?>
</div>
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:1rem">
  <?php
  $avantages=[
    ['💳','Cashback 1,5%','Sur tous vos achats avec la carte Gold','premium'],
    ['✈️','Lounges aéroports','Accès gratuit + invité','premium'],
    ['🏨','Hôtels partenaires','-15% dans 2000 hôtels du monde','standard'],
    ['🛡️','Assurance voyage','Couverture mondiale incluse','premium'],
    ['📱','Assistance 24/7','Ligne dédiée conseiller personnel','premium'],
    ['🎁','Offres partenaires','Réductions exclusives shopping, loisirs','standard'],
  ];
  foreach($avantages as $a): $ok=$a[3]==='standard'||$plan==='premium'; ?>
  <div class="card card-pad" style="<?php echo $ok?'':'opacity:.5;';?>">
    <div style="font-size:1.5rem;margin-bottom:.75rem"><?php echo $a[0];?></div>
    <div style="font-family:'Sora',sans-serif;font-weight:700;margin-bottom:.3rem;color:var(--bnp-dark)"><?php echo $a[1];?></div>
    <div style="font-size:.8rem;color:var(--gray600);margin-bottom:.85rem"><?php echo $a[2];?></div>
    <?php if($ok):?><button class="btn btn-outline btn-sm" onclick="showToast('Avantage activé','success')">Activer</button>
    <?php else:?><span class="badge badge-gold">Premium requis</span><?php endif;?>
  </div>
  <?php endforeach;?>
</div>
