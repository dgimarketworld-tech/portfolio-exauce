...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../investissement/index.php" class="sidebar-link active"><span class="sidebar-icon">◈</span> Investissements </a>
...
<?php
$title='Investissements'; $breadcrumb='Épargne · Investissements';
DB::pdo()->exec("CREATE TABLE IF NOT EXISTS investissements (
    id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    user_id INT UNSIGNED NOT NULL,
    type_investissement VARCHAR(60) NOT NULL,
    montant DECIMAL(12,2) NOT NULL,
    rendement_annuel DECIMAL(5,2) DEFAULT 0,
...
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrf($_POST['_csrf']??'')){
    ...
    $rendement=['GTB Performance'=>6.8,'GTB Securise'=>2.5,'GTB Premium Plus'=>12.0][$type_inv]??0;
    if($montant_inv<100){$inv_error='Montant minimum : 100 €.';}
    else{
        $compte=DB::one("SELECT * FROM comptes WHERE user_id=:uid AND statut='actif' ORDER BY solde DESC LIMIT 1",['uid'=>$uid]);
        if(!$compte||$compte['solde']<$montant_inv){$inv_error='Solde insuffisant.';}
        else{
...
?>
<div class="sec-head"><div><h1 class="pg-title">Investissements</h1><p class="pg-sub">Faites fructifier votre épargne</p></div></div>
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:1.25rem;margin-bottom:1.5rem">
  <div class="card card-pad" style="border-color:var(--bnp-emerald)">
    <div style="display:flex;align-items:center;gap:.85rem;margin-bottom:1rem"><div style="font-size:1.5rem">📈</div><div><div style="font-family:'Sora',sans-serif;font-weight:700;color:var(--bnp-dark)">GTB Performance</div><div style="font-size:.72rem;color:var(--bnp-emerald);font-weight:600">Rendement moyen : +6,8%/an</div></div></div>
    <div style="font-size:.82rem;color:var(--gray600);margin-bottom:1rem;line-height:1.6">Portefeuille diversifié — actions mondiales, obligations, ETF. Idéal pour un horizon 5+ ans.</div>
    <div style="display:flex;justify-content:space-between;font-size:.78rem;margin-bottom:1rem"><span style="color:var(--gray400)">Risque</span><span class="badge badge-gold">Modéré</span></div>
    <button class="btn btn-primary btn-sm" onclick="showToast('Redirection vers votre conseiller…','info')">Souscrire</button>
  </div>
  <div class="card card-pad">
    <div style="display:flex;align-items:center;gap:.85rem;margin-bottom:1rem"><div style="font-size:1.5rem">🛡️</div><div><div style="font-family:'Sora',sans-serif;font-weight:700;color:var(--bnp-dark)">GTB Sécurisé</div><div style="font-size:.72rem;color:var(--green);font-weight:600">Rendement garanti : +2,5%/an</div></div></div>
    <div style="font-size:.82rem;color:var(--gray600);margin-bottom:1rem;line-height:1.6">Capital garanti, rendement fixe. Fonds obligataires et monétaires. Idéal débutants.</div>
    <div style="display:flex;justify-content:space-between;font-size:.78rem;margin-bottom:1rem"><span style="color:var(--gray400)">Risque</span><span class="badge badge-green">Faible</span></div>
    <button class="btn btn-outline btn-sm" onclick="showToast('Redirection vers votre conseiller…','info')">Souscrire</button>
  </div>
  <div class="card card-pad">
    <div style="display:flex;align-items:center;gap:.85rem;margin-bottom:1rem"><div style="font-size:1.5rem">⚡</div><div><div style="font-family:'Sora',sans-serif;font-weight:700;color:var(--bnp-dark)">GTB Premium Plus</div><div style="font-size:.72rem;color:var(--bnp-emerald);font-weight:600">Potentiel : +12%/an</div></div></div>
    <div style="font-size:.82rem;color:var(--gray600);margin-bottom:1rem;line-height:1.6">Fonds d'investissement actifs, private equity, marchés émergents. Réservé clients Premium.</div>
    <div style="display:flex;justify-content:space-between;font-size:.78rem;margin-bottom:1rem"><span style="color:var(--gray400)">Risque</span><span class="badge badge-red">Élevé</span></div>
    <button class="btn btn-outline btn-sm" onclick="showToast('Contacter votre conseiller pour ce produit','info')">Contacter</button>
  </div>
</div>
<?php if($inv_error):?><div style="background:rgba(229,55,58,.08);border:1px solid rgba(229,55,58,.2);border-radius:var(--r-md);padding:.85rem;margin-bottom:1.25rem;font-size:.82rem;color:var(--red)">❌ <?php echo e($inv_error);?></div><?php endif;?>
<?php if($inv_success):?><div style="background:rgba(0,198,122,.08);border:1px solid rgba(0,198,122,.2);border-radius:var(--r-md);padding:.85rem;margin-bottom:1.25rem;font-size:.82rem;color:var(--green)">✅ <?php echo e($inv_success);?></div><?php endif;?>
<div class="card card-pad" style="margin-bottom:1.5rem">
  <div class="card-header"><div class="card-title">Mes positions</div></div>
  <?php if(empty($investissements)):?>
  <div style="text-align:center;padding:2rem;color:var(--gray400)">
    <div style="font-size:2rem;margin-bottom:.75rem">📊</div>
    Aucun investissement actif. <a href="#souscrire" style="color:var(--bnp-emerald)">Démarrer maintenant</a>
  </div>
  <?php else:?>
  <div class="tbl-wrap"><table class="tbl"><thead><tr><th>Type</th><th>Montant</th><th>Rendement</th><th>Début</th><th>Échéance</th><th>Statut</th></tr></thead><tbody>
  <?php foreach($investissements as $inv):?>
  <tr><td><?php echo e($inv['produit']);?></td><td><?php echo number_format((float)$inv['montant_initial'],2,',',' ').' €';?></td><td><?php echo $inv['rendement_pct'];?>%</td><td><?php echo date('d/m/Y',strtotime($inv['date_debut']));?></td><td><?php echo $inv['date_fin']?date('d/m/Y',strtotime($inv['date_fin'])):'-';?></td><td><span class="badge <?php echo $inv['statut']==='actif'?'badge-green':($inv['statut']==='cloture'?'badge-gray':'badge-gold');?>"><?php echo ucfirst($inv['statut']);?></span></td></tr>
  <?php endforeach;?>
  </tbody></table></div>
  <?php endif;?>
</div>
<div class="card card-pad" id="souscrire">
  <div class="card-header"><div class="card-title">Souscrire un investissement</div></div>
  <form method="POST" style="max-width:480px">
    <input type="hidden" name="_csrf" value="<?php echo e(csrf_token());?>"/>
    <div class="form-group"><label class="form-label">Produit</label>
      <select name="type_inv" class="form-select">
        <option value="GTB Performance">GTB Performance (+6,8%/an)</option>
        <option value="GTB Securise">GTB Sécurisé (+2,5%/an garanti)</option>
        <option value="GTB Premium Plus">GTB Premium Plus (+12%/an)</option>
      </select></div>
    <div class="form-group"><label class="form-label">Montant à investir (€)</label><input class="form-input" name="montant_inv" type="number" step="0.01" min="100" placeholder="1000" required/></div>
