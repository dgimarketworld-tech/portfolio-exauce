...
<header class="topbar"><div class="topbar-left"><button class="sidebar-toggle" id="sidebarToggle">☰</button><div><div class="topbar-page-title"><?php echo $title??'';?></div></div></div></header>
<main class="dash-main">
<?php
$title='Simulateur de crédit'; $breadcrumb='Crédits · Simulateur';
?>
<div class="sec-head"><div><a href="index.php" style="font-size:.8rem;color:var(--gray400);text-decoration:none">← Retour</a><h1 class="pg-title" style="margin-top:.4rem">Simulateur de crédit</h1></div></div>
<div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;align-items:start">
  <div class="card card-pad">
    <div class="form-group"><label class="form-label">Type de crédit</label>
      <select class="form-select" id="simType" onchange="calcSim()">
    ...
    <div class="form-group"><label class="form-label">Montant (€)</label>
      <input class="form-input" type="number" id="simAmt" value="25000" oninput="calcSim()"/></div>
    <div class="form-group"><label class="form-label">Durée : <b id="durLbl">60 mois</b></label>
      <input type="range" id="simDur" min="12" max="300" value="60" style="width:100%;accent-color:var(--bnp-emerald)" oninput="document.getElementById('durLbl').textContent=this.value+' mois';calcSim()"/>
      <div style="display:flex;justify-content:space-between;font-size:.7rem;color:var(--gray400);margin-top:.25rem"><span>1 an</span><span>25 ans</span></div></div>
    <a href="demande.php" class="btn btn-primary" style="width:100%;justify-content:center">Faire une demande →</a>
  </div>
  <div class="card card-pad">
    <div class="card-title" style="margin-bottom:1.5rem">Résultats de la simulation</div>
    <div style="display:flex;flex-direction:column;gap:1rem">
      <div style="background:linear-gradient(135deg,var(--bnp-deeper),#1a2e40);border-radius:var(--r-lg);padding:1.25rem;text-align:center">
        <div style="font-size:.68rem;color:rgba(255,255,255,.4);text-transform:uppercase;letter-spacing:.08em;margin-bottom:.35rem">Mensualité estimée</div>
        <div style="font-family:'Sora',sans-serif;font-weight:800;font-size:2rem;color:#fff" id="rMens">—</div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem">
        <div style="background:var(--gray50);border-radius:var(--r-md);padding:1rem;text-align:center">
          <div style="font-size:.68rem;color:var(--gray400);text-transform:uppercase;letter-spacing:.06em;margin-bottom:.25rem">Taux TEG</div>
          <div style="font-family:'Sora',sans-serif;font-weight:700;font-size:1.1rem;color:var(--bnp-emerald)" id="rTaux">—</div>
        </div>
        <div style="background:var(--gray50);border-radius:var(--r-md);padding:1rem;text-align:center">
          <div style="font-size:.68rem;color:var(--gray400);text-transform:uppercase;letter-spacing:.06em;margin-bottom:.25rem">Coût total</div>
          <div style="font-family:'Sora',sans-serif;font-weight:700;font-size:1.1rem;color:var(--gray800)" id="rTotal">—</div>
        </div>
        <div style="background:var(--gray50);border-radius:var(--r-md);padding:1rem;text-align:center">
          <div style="font-size:.68rem;color:var(--gray400);text-transform:uppercase;letter-spacing:.06em;margin-bottom:.25rem">Coût des intérêts</div>
          <div style="font-family:'Sora',sans-serif;font-weight:700;font-size:1.1rem;color:var(--gray800)" id="rInterets">—</div>
        </div>
        <div style="background:var(--gray50);border-radius:var(--r-md);padding:1rem;text-align:center">
          <div style="font-size:.68rem;color:var(--gray400);text-transform:uppercase;letter-spacing:.06em;margin-bottom:.25rem">Durée</div>
          <div style="font-family:'Sora',sans-serif;font-weight:700;font-size:1.1rem;color:var(--gray800)" id="rDuree">—</div>
        </div>
      </div>
    </div>
  </div>
</div>
</main></div><div class="toast-container" id="toastContainer"></div><script>(function(){'use strict';const sb=document.getElementById('sidebar'),tgl=document.getElementById('sidebarToggle'),ov=document.getElementById('sidebarOverlay');function openSb(){sb?.classList.add('open');ov?.classList.add('show');document.body.style.overflow='hidden';}function closeSb(){sb?.classList.remove('open');ov?.classList.remove('show');document.body.style.overflow='';}tgl?.addEventListener('click',()=>sb?.classList.contains('open')?closeSb():openSb());ov?.addEventListener('click',closeSb);window.showToast=function(m,t){const c=document.getElementById('toastContainer');if(!c)return;const ic={success:'✓',error:'✕',info:'ℹ',warning:'⚠'};const x=document.createElement('div');x.className='toast '+(t||'success');x.innerHTML=`<span class="toast-icon">${ic[t||'success']||'•'}</span><span>${m}</span>`;c.appendChild(x);requestAnimationFrame(()=>setTimeout(()=>x.classList.add('show'),25));setTimeout(()=>{x.classList.remove('show');setTimeout(()=>x.remove(),400);},3600);};})();</script><script src="../app-nav.js"></script>
</body></html>
<script>
function calcSim(){
  const rate=parseFloat(document.getElementById('simType').value)||0;
  const amt=parseFloat(document.getElementById('simAmt').value)||0;
  const dur=parseInt(document.getElementById('simDur').value)||60;
  if(!amt||!rate){return;}
  const r=rate/100/12;
  const mens=r===0?amt/dur:amt*r*Math.pow(1+r,dur)/(Math.pow(1+r,dur)-1);
  const total=mens*dur;
  document.getElementById('rMens').textContent=mens.toFixed(2)+' €';
  document.getElementById('rTaux').textContent=rate.toFixed(1)+'%';
  document.getElementById('rTotal').textContent=Math.round(total).toLocaleString('fr-FR')+' €';
  document.getElementById('rInterets').textContent=Math.round(total-amt).toLocaleString('fr-FR')+' €';
  document.getElementById('rDuree').textContent=dur+' mois';
}
calcSim();
</script>
