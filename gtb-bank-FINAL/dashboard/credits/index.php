...
<?php
$title='Mes Crédits'; $breadcrumb='Crédits en cours';
...
?>
<div class="sec-head"><div><h1 class="pg-title">Mes Crédits</h1><p class="pg-sub"><?php echo count($actifs);?> crédit(s) en cours</p></div>
<a href="demande.php" class="btn btn-primary btn-sm">+ Nouvelle demande</a></div>
<?php if(empty($credits)):?>
<div class="card card-pad" style="text-align:center;padding:3rem">
  <div style="font-size:2rem;margin-bottom:.75rem">📋</div>
  <div style="font-family:'Sora',sans-serif;font-weight:700;color:var(--bnp-dark)">Aucun crédit</div>
  <a href="demande.php" class="btn btn-primary btn-sm" style="margin-top:1rem">Faire une demande</a>
</div>
<?php else:?>
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:1.25rem">
<?php foreach($credits as $cr):
  $pct=round((($cr['montant']-$cr['solde_restant'])/$cr['montant'])*100);
?>
<div class="card card-pad">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:1rem">
    <div>
      <div style="font-size:.68rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--gray400)"><?php echo ucfirst($cr['type']);?></div>
      <div style="font-family:'Sora',sans-serif;font-weight:800;font-size:1.2rem;color:var(--bnp-dark);margin-top:.2rem"><?php echo format_money($cr['montant']);?></div>
    </div>
    <span class="badge badge-<?php echo ['en_cours'=>'green','en_etude'=>'blue','accepte'=>'blue','refuse'=>'red','solde'=>'gray'][$cr['statut']]??'gray';?>"><?php echo ucfirst(str_replace('_',' ',$cr['statut']));?></span>
  </div>
  <?php if($cr['statut']==='en_cours'):?>
  <div style="margin-bottom:1rem">
    <div style="display:flex;justify-content:space-between;font-size:.72rem;color:var(--gray400);margin-bottom:.35rem"><span>Remboursé</span><span><?php echo $pct;?>%</span></div>
    <div style="height:6px;background:var(--gray100);border-radius:99px;overflow:hidden"><div style="width:<?php echo $pct;?>%;height:100%;background:var(--bnp-emerald);border-radius:99px;transition:width .5s"></div></div>
  </div>
  <div style="display:flex;justify-content:space-between;font-size:.82rem;margin-bottom:.85rem">
    <div><div style="color:var(--gray400);font-size:.7rem">Mensualité</div><div style="font-weight:600"><?php echo format_money($cr['mensualite']);?></div></div>
    <div style="text-align:right"><div style="color:var(--gray400);font-size:.7rem">Restant</div><div style="font-weight:600"><?php echo format_money($cr['solde_restant']);?></div></div>
  </div>
  <div style="font-size:.72rem;color:var(--gray400)">Réf. <?php echo e($cr['reference']);?> · <?php echo $cr['duree_mois'];?> mois · <?php echo $cr['taux'];?>%</div>
  <?php endif;?>
</div>
<?php endforeach;?>
</div>
