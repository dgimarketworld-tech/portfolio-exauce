...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../cartes/index.php" class="sidebar-link active"><span class="sidebar-icon">▣</span> Cartes </a>
...
<?php
$title='Demande de carte'; $breadcrumb='Cartes · Nouvelle demande';
$success=''; $error='';
...
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrf($_POST['_csrf']??'')){
    $type=$_POST['type']??''; $reseau=$_POST['reseau']??''; $compte_id=(int)($_POST['compte_id']??0);
    $compte=DB::one("SELECT * FROM comptes WHERE id=:id AND user_id=:uid",['id'=>$compte_id,'uid'=>Session::userId()]);
    if(!$type||!$reseau||!$compte){$error='Veuillez remplir tous les champs.';}
    else{
        ...
        send_notification_email(Session::userId(),'Demande de carte bancaire — GTB Bank',"Votre demande de carte <strong>".strtoupper($type)."</strong> a été enregistrée. Elle sera disponible sous <strong>3 à 5 jours ouvrés</strong>.");
        ...
        $html_adm="<div style='font-family:Arial,sans-serif;max-width:520px;margin:auto;padding:32px;border:1px solid #e5e7eb;border-radius:8px'><h2 style='color:#1a3c5e'>Nouvelle demande de carte</h2><table style='width:100%;border-collapse:collapse;color:#374151'><tr><td style='padding:6px 0;font-weight:600'>Client</td><td>{$u['prenom']} {$u['nom']} ({$u['email']})</td></tr><tr><td style='padding:6px 0;font-weight:600'>Type de carte</td><td>".strtoupper($type)." / ".strtoupper($reseau)."</td></tr><tr><td style='padding:6px 0;font-weight:600'>Numéro masqué</td><td>$masque</td></tr><tr><td style='padding:6px 0;font-weight:600'>Date</td><td>".date('d/m/Y H:i')."</td></tr></table><a href='".GTB_BASE_URL."/admin/utilisateurs/index.php' style='display:inline-block;margin:20px 0;padding:12px 24px;background:#1a3c5e;color:#fff;text-decoration:none;border-radius:6px;font-weight:600'>Voir le client</a><p style='color:#9ca3af;font-size:12px'>Global Trust Bank — Notification automatique</p></div>";
...
$csrf=csrf_token();
?>
<div class="sec-head"><div><a href="index.php" style="font-size:.8rem;color:var(--gray400);text-decoration:none">← Retour aux cartes</a><h1 class="pg-title" style="margin-top:.4rem">Demander une carte</h1></div></div>
<?php if($success):?>
<div style="background:rgba(0,198,122,.08);border:1px solid rgba(0,198,122,.2);border-radius:var(--r-xl);padding:2rem;text-align:center">
  ...
  <div style="font-family:'Sora',sans-serif;font-weight:800;font-size:1.1rem;color:var(--bnp-dark);margin-bottom:.5rem">Demande enregistrée !</div>
  <div style="font-size:.85rem;color:var(--gray600);margin-bottom:1.5rem"><?php echo e($success);?></div>
  <a href="index.php" class="btn btn-primary">Voir mes cartes</a>
</div>
<?php else:?>
<?php if($error):?><div style="background:rgba(229,55,58,.08);border:1px solid rgba(229,55,58,.2);border-radius:var(--r-md);padding:.85rem;margin-bottom:1.25rem;color:var(--red);font-size:.82rem"><?php echo e($error);?></div><?php endif;?>
<div style="display:grid;grid-template-columns:1fr 280px;gap:1.5rem">
  <div class="card card-pad">
    <form method="POST">
      <input type="hidden" name="_csrf" value="<?php echo e($csrf);?>"/>
      <div class="form-group"><label class="form-label">Compte à associer</label>
        <select name="compte_id" class="form-select" required>
          <?php foreach($comptes as $c):?><option value="<?php echo $c['id'];?>"><?php echo ucfirst($c['type']).' — '.e(substr($c['numero'],-8)).' — '.format_money($c['solde'],$c['devise']);?></option><?php endforeach;?>
        </select></div>
      <div class="form-group"><label class="form-label">Type de carte</label>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem">
          <label style="cursor:pointer"><input type="radio" name="type" value="standard" required style="margin-right:.5rem"/>Standard (gratuit)</label>
          <label style="cursor:pointer"><input type="radio" name="type" value="gold" style="margin-right:.5rem"/>Gold (3,90€/mois)</label>
          <label style="cursor:pointer"><input type="radio" name="type" value="infinite" style="margin-right:.5rem"/>Infinite (9,90€/mois)</label>
          <label style="cursor:pointer"><input type="radio" name="type" value="business" style="margin-right:.5rem"/>Business (7,90€/mois)</label>
        </div></div>
      <div class="form-group"><label class="form-label">Réseau</label>
        <div style="display:flex;gap:1.5rem">
          <label style="cursor:pointer"><input type="radio" name="reseau" value="visa" required style="margin-right:.5rem"/>Visa</label>
          <label style="cursor:pointer"><input type="radio" name="reseau" value="mastercard" style="margin-right:.5rem"/>Mastercard</label>
        </div></div>
      <button type="submit" class="btn btn-primary" style="width:100%">Envoyer la demande →</button>
    </form>
  </div>
  <div class="card card-pad">
    <div class="card-title" style="margin-bottom:1rem">Comparatif</div>
    <div style="display:flex;flex-direction:column;gap:.5rem;font-size:.82rem">
      <div style="display:flex;justify-content:space-between;padding:.5rem 0;border-bottom:1px solid var(--gray50)"><span>Standard</span><span class="badge badge-green">Gratuit</span></div>
      <div style="display:flex;justify-content:space-between;padding:.5rem 0;border-bottom:1px solid var(--gray50)"><span>Gold</span><span>3,90 €/mois</span></div>
      <div style="display:flex;justify-content:space-between;padding:.5rem 0;border-bottom:1px solid var(--gray50)"><span>Infinite</span><span>9,90 €/mois</span></div>
      <div style="display:flex;justify-content:space-between;padding:.5rem 0"><span>Business</span><span>7,90 €/mois</span></div>
    </div>
  </div>
</div>
<?php endif;?>
