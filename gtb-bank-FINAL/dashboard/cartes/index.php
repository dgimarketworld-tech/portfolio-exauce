...
if($_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['action'])&&in_array($_POST['action'],['block','unblock'],true)){
    Security::verifyCsrf($_POST['csrf_token']??'');
    $card_id=(int)($_POST['card_id']??0);
    $new_status=$_POST['action']==='block'?'bloquee':'active';
    DB::update('cartes',['statut'=>$new_status],'id=:id AND user_id=:uid',['id'=>$card_id,'uid'=>Session::userId()]);
...
<?php
$title='Mes Cartes'; $breadcrumb='Tableau de bord · Cartes';
$cartes=DB::all("SELECT ca.*,co.numero as compte_numero,co.type as compte_type FROM cartes ca JOIN comptes co ON ca.compte_id=co.id WHERE co.user_id=:id ORDER BY ca.statut,ca.cree_le DESC",['id'=>Session::userId()]);
?>
<div class="sec-head"><div><h1 class="pg-title">Mes Cartes</h1><p class="pg-sub"><?php echo count($cartes);?> carte(s)</p></div>
<a href="demande.php" class="btn btn-primary btn-sm">+ Demander une carte</a></div>

<?php if(empty($cartes)):?>
<div class="card card-pad" style="text-align:center;padding:3rem">
  <div style="font-size:3rem;margin-bottom:1rem">💳</div>
  <div style="font-family:'Sora',sans-serif;font-weight:700;color:var(--bnp-dark)">Aucune carte active</div>
  <a href="demande.php" class="btn btn-primary" style="margin-top:1rem">Demander ma première carte</a>
</div>
<?php else:?>
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(320px,1fr));gap:1.5rem">
<?php foreach($cartes as $c): $active=$c['statut']==='active'; ?>
  <div>
    <!-- Visuel carte -->
    <div style="background:linear-gradient(135deg,<?php echo $c['type']==='gold'?'#B8860B,#FFD700':($c['type']==='infinite'?'#1a1a2e,#16213e':'var(--bnp-deeper),#1a2e40');?>);border-radius:var(--r-xl);padding:1.5rem;color:#fff;margin-bottom:1rem;position:relative;overflow:hidden">
      ...
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:1.5rem">
        <div style="font-size:.72rem;letter-spacing:.1em;text-transform:uppercase;color:rgba(255,255,255,.5)"><?php echo strtoupper($c['type']);?> <?php echo strtoupper($c['reseau']);?></div>
        <span style="font-size:.72rem;padding:.25rem .6rem;border-radius:99px;background:<?php echo $active?'rgba(0,198,122,.2)':'rgba(229,55,58,.2)';?>;color:<?php echo $active?'#4ade80':'#f87171';?>"><?php echo $active?'● Active':'● Bloquée';?></span>
      </div>
      <div style="font-family:monospace;font-size:1rem;letter-spacing:.2em;margin-bottom:1.5rem"><?php echo e($c['numero_masque']);?></div>
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div style="font-size:.7rem;color:rgba(255,255,255,.4)">Expire <b style="color:rgba(255,255,255,.7)"><?php echo date('m/y',strtotime($c['expire_le']));?></b></div>
        <?php if($c['plafond']):?><div style="font-size:.7rem;color:rgba(255,255,255,.4)">Plafond <b style="color:rgba(255,255,255,.7)"><?php echo format_money($c['plafond']);?></b></div><?php endif;?>
      </div>
    </div>
    <!-- Actions -->
    <div class="card card-pad">
      <div style="font-size:.72rem;color:var(--gray400);margin-bottom:.85rem">Compte lié : <?php echo ucfirst($c['compte_type']);?> — <?php echo e(substr($c['compte_numero'],-8));?></div>
      <div style="display:flex;gap:.5rem;flex-wrap:wrap">
        <?php if($active):?>
        <form method="POST" style="display:inline">
          <input type="hidden" name="csrf_token" value="<?php echo e(csrf_token());?>"/>
          <input type="hidden" name="card_id" value="<?php echo (int)$c['id'];?>"/>
          <input type="hidden" name="action" value="block"/>
          <button type="submit" class="btn btn-outline btn-sm">🔒 Bloquer</button>
        </form>
        <button class="btn btn-outline btn-sm" onclick="showToast('Code PIN envoyé par SMS','info')">🔑 Voir PIN</button>
        <button class="btn btn-outline btn-sm" onclick="showToast('Opposition enregistrée — contactez le support','warning')">⛔ Opposition</button>
        <?php else:?>
        <form method="POST" style="display:inline">
          <input type="hidden" name="csrf_token" value="<?php echo e(csrf_token());?>"/>
          <input type="hidden" name="card_id" value="<?php echo (int)$c['id'];?>"/>
