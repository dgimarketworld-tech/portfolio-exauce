...
<aside class="sidebar" id="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="../notifications/index.php" class="nav-link active"><span class="nav-icon">🔔</span> Notifications</a>
...
if ($_SERVER['REQUEST_METHOD']==='POST' && Security::csrfCheck($_POST['_csrf']??'')) {
    ...
    // Envoyer une notification à un ou tous les utilisateurs
    if ($action==='send') {
        $target  = $_POST['target']??'all';
        $type    = $_POST['type']??'info';
        $title_n = trim($_POST['title']??'');
        $message = trim($_POST['message']??'');
        if (!$title_n||!$message) { $error='Titre et message requis.'; }
        else {
            if ($target==='all') {
                $users = DB::all("SELECT id FROM users WHERE is_active=1");
                foreach($users as $u) notify($u['id'],$title_n,$message,$type);
                $success='Notification envoyée à '.count($users).' utilisateurs.';
            } else {
                $uid=(int)$target;
                notify($uid,$title_n,$message,$type);
    ...
    elseif ($action==='read_all') {
        DB::update("UPDATE notifications SET is_read=1");
        $success='Toutes les notifications marquées comme lues.';
    }
    // Supprimer toutes
    elseif ($action==='delete_all') {
        DB::update("DELETE FROM notifications WHERE is_read=1");
        $success='Notifications lues supprimées.';
    }
}

$filter = $_GET['type']??'all';
$where  = $filter!=='all' ? "WHERE n.type='$filter'" : '';
$notifs = DB::all("SELECT n.*,u.prenom,u.nom,u.email FROM notifications n LEFT JOIN users u ON u.id=n.user_id $where ORDER BY n.cree_le DESC LIMIT 100");
$total  = (int)DB::scalar("SELECT COUNT(*) FROM notifications");
$unread = (int)DB::scalar("SELECT COUNT(*) FROM notifications WHERE is_read=0");
$users  = DB::all("SELECT id, CONCAT(COALESCE(prenom,first_name,''),' ',COALESCE(nom,last_name,'')) AS name, email FROM users WHERE is_active=1 ORDER BY prenom");
?>

<div class="page-header">
  <div class="page-header-top">
    <div><h1 class="page-title">Notifications</h1><p class="page-sub">Gérez et envoyez des notifications aux clients</p></div>
    <div class="page-actions">
      <button class="btn btn-outline btn-sm" onclick="openModal('modal-send')">📢 Envoyer une notification</button>
      ...
      <form method="POST" style="display:inline" onsubmit="return confirm('Supprimer les notifications lues ?')">
        <input type="hidden" name="_csrf" value="<?=$csrf?>">
        <input type="hidden" name="_action" value="delete_all">
        <button type="submit" class="btn btn-danger btn-sm">🗑 Nettoyer</button>
      </form>
    </div>
  </div>
</div>
...
<div class="admin-kpi-grid" style="margin-bottom:1.5rem">
  <div class="kpi-card"><div class="kpi-icon" style="background:rgba(59,130,246,.1);color:var(--accent)">🔔</div><div><div class="kpi-label">Total</div><div class="kpi-value"><?=$total?></div></div></div>
  <div class="kpi-card"><div class="kpi-icon" style="background:rgba(239,68,68,.1);color:var(--danger)">🔴</div><div><div class="kpi-label">Non lues</div><div class="kpi-value"><?=$unread?></div></div></div>
  <div class="kpi-card"><div class="kpi-icon" style="background:rgba(16,185,129,.1);color:var(--success)">✓</div><div><div class="kpi-label">Lues</div><div class="kpi-value"><?=$total-$unread?></div></div></div>
</div>

<div class="admin-card">
  <div class="card-header">
    <div class="card-title">Historique des notifications</div>
    <div style="display:flex;gap:.4rem;flex-wrap:wrap">
      <?php foreach(['all'=>'Toutes','info'=>'Info','success'=>'Succès','warning'=>'Avertissement','error'=>'Erreur'] as $k=>$v):?>
  ...
  <div style="overflow-x:auto">
    <table class="admin-table">
      <thead><tr><th>Client</th><th>Type</th><th>Titre</th><th>Message</th><th>Date</th><th>Lu</th></tr></thead>
      <tbody>
      <?php if(empty($notifs)):?>
      <tr><td colspan="6" style="text-align:center;padding:2rem;color:var(--gray400)">Aucune notification</td></tr>
      <?php else: foreach($notifs as $n):
        $typeColors=['info'=>'badge-blue','success'=>'badge-success','warning'=>'badge-warning','error'=>'badge-danger'];
        $tc=$typeColors[$n['type']]??'badge-gray';
      ?>
      <tr style="<?=!$n['is_read']?'background:rgba(59,130,246,.03)':''?>">
        <td><div style="font-size:.78rem;font-weight:600"><?=htmlspecialchars(trim(($n['prenom']??'').' '.($n['nom']??''))?:($n['email']??'—'))?></div><div style="font-size:.68rem;color:var(--gray400)"><?=htmlspecialchars($n['email']??'')?></div></td>
        <td><span class="badge <?=$tc?>"><?=ucfirst($n['type'])?></span></td>
        <td style="font-weight:600;font-size:.8rem"><?=htmlspecialchars($n['title'])?></td>
        <td style="font-size:.76rem;color:var(--gray600);max-width:250px"><?=htmlspecialchars($n['message'])?></td>
        <td style="font-size:.72rem;color:var(--gray500);white-space:nowrap"><?=date('d/m/Y H:i',strtotime($n['cree_le']))?></td>
        <td><?=$n['is_read']?'<span class="badge badge-success">Lu</span>':'<span class="badge badge-danger">Non lu</span>'?></td>
      </tr>
...
<div class="modal-overlay" id="modal-send">
  <div class="modal">
    <div class="modal-head">
      <div><div class="modal-title">Envoyer une notification</div><div class="modal-sub">À un client ou à tous</div></div>
    ...
    <form method="POST">
      ...
      <div class="modal-body">
        ...
        <div class="form-group"><label class="form-label">Titre</label><input class="form-input" name="title" required placeholder="Maintenance planifiée"></div>
        <div class="form-group"><label class="form-label">Message</label><textarea class="form-textarea" name="message" required placeholder="Votre message ici..."></textarea></div>
      </div>
      <div class="modal-foot">
        <button type="button" class="btn btn-outline" onclick="closeModal('modal-send')">Annuler</button>
