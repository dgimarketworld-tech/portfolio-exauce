...
<?php
$title='Notifications'; $breadcrumb='Tableau de bord · Notifications';
// Mark all as read
if($_GET['action']??''==='read_all'){DB::update("UPDATE notifications SET is_read=1 WHERE user_id=:id",['id'=>Session::userId()]);header('Location: notifications.php');exit;}
...
?>
<div class="sec-head"><div><h1 class="pg-title">Notifications</h1><p class="pg-sub"><?php echo count($unread); ?> non lue(s)</p></div>
<?php if(count($unread)>0): ?><a href="?action=read_all" class="btn btn-outline btn-sm">Tout marquer comme lu</a><?php endif; ?>
</div>
<div class="card card-pad">
<?php if(empty($notifs)): ?>
<div style="text-align:center;padding:3rem;color:var(--gray400)"><div style="font-size:2rem;margin-bottom:.75rem">🔔</div>Aucune notification</div>
<?php else: ?>
<?php foreach($notifs as $n): ?>
<div class="row-item" style="<?php echo !$n['is_read']?'background:rgba(212,175,55,.04);':''; ?>">
  <div class="row-icon" style="background:<?php echo ['success'=>'#F0FDF4','error'=>'#FEF2F2','warning'=>'#FFF8E1'][$n['type']]??'#EFF6FF'; ?>">
    <?php echo ['success'=>'✅','error'=>'❌','warning'=>'⚠️'][$n['type']]??'ℹ️'; ?>
  </div>
  <div class="row-info">
    <div class="row-title"><?php echo e($n['title']); ?><?php if(!$n['is_read']): ?> <span style="width:7px;height:7px;border-radius:50%;background:var(--bnp-emerald);display:inline-block;margin-left:.4rem;vertical-align:middle"></span><?php endif; ?></div>
    <div class="row-sub"><?php echo e($n['message']); ?></div>
    <div style="font-size:.68rem;color:var(--gray400);margin-top:.2rem"><?php echo time_ago($n['cree_le']); ?></div>
  </div>
  <?php if($n['link_url']): ?><a href="<?php echo e($n['link_url']); ?>" class="btn btn-outline btn-xs"><?php echo e($n['link_label']??'Voir'); ?></a><?php endif; ?>
</div>
<?php endforeach; ?>
<?php endif; ?>
</div>
