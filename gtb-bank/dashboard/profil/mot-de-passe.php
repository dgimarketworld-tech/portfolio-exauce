...
<?php
$title='Mot de passe'; $breadcrumb='Profil · Sécurité';
$error=''; $success='';
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrf($_POST['_csrf']??'')){
    $old=$_POST['old_pw']??''; $new=$_POST['new_pw']??''; $confirm=$_POST['confirm_pw']??'';
    $user=DB::one("SELECT password_hash FROM users WHERE id=:id",['id'=>Session::userId()]);
    if(!$user||!password_verify($old,$user['password_hash'])){$error='Mot de passe actuel incorrect.';}
    elseif(strlen($new)<8){$error='Minimum 8 caractères.';}
...
$csrf=csrf_token();
?>
<div class="sec-head"><div><a href="index.php" style="font-size:.8rem;color:var(--gray400);text-decoration:none">← Retour au profil</a><h1 class="pg-title" style="margin-top:.4rem">Changer le mot de passe</h1></div></div>
...
<div style="display:grid;grid-template-columns:1fr 360px;gap:1.5rem;align-items:start">
  <div class="card card-pad">
    <form method="POST">
      ...
      <div class="form-group"><label class="form-label">Mot de passe actuel</label><input class="form-input" type="password" name="old_pw" required/></div>
      <div style="height:1px;background:var(--gray100);margin:1rem 0"></div>
      <div class="form-group"><label class="form-label">Nouveau mot de passe</label><input class="form-input" type="password" name="new_pw" id="newPw" oninput="checkPw(this.value)" required/>
        <div style="margin-top:.6rem"><div style="height:5px;background:var(--gray100);border-radius:99px;overflow:hidden;margin-bottom:.3rem"><div id="pwBar" style="height:100%;border-radius:99px;width:0;transition:all .4s;background:var(--red)"></div></div>
        <div id="pwLbl" style="font-size:.72rem;color:var(--gray400)">Saisissez un mot de passe</div></div>
      </div>
      <div class="form-group"><label class="form-label">Confirmer</label><input class="form-input" type="password" name="confirm_pw" required/></div>
      <button type="submit" class="btn btn-primary" style="width:100%">🔒 Enregistrer</button>
    </form>
  </div>
  <div class="card card-pad"><div class="card-title" style="margin-bottom:1rem">Critères requis</div>
    <div class="pw-cr" id="cr-len">○ Minimum 8 caractères</div>
    <div class="pw-cr" id="cr-maj">○ Au moins une majuscule</div>
    <div class="pw-cr" id="cr-num">○ Au moins un chiffre</div>
    <div class="pw-cr" id="cr-spe">○ Un caractère spécial</div>
  </div>
</div>
...
<style>.pw-cr{font-size:.82rem;color:var(--gray400);padding:.38rem 0;transition:color .3s}.pw-cr.ok{color:var(--green)}</style>
<script>
function checkPw(v){
  const len=v.length>=8,maj=/[A-Z]/.test(v),num=/[0-9]/.test(v),spe=/[^A-Za-z0-9]/.test(v);
  ['len','maj','num','spe'].forEach((k,i)=>{document.getElementById('cr-'+k)?.classList.toggle('ok',[len,maj,num,spe][i]);});
  const sc=[len,maj,num,spe].filter(Boolean).length;
  const lvl=[{w:'10%',c:'var(--red)',t:'Très faible'},{w:'30%',c:'#F97316',t:'Faible'},{w:'60%',c:'#EAB308',t:'Moyen'},{w:'85%',c:'var(--green)',t:'Fort'},{w:'100%',c:'#059669',t:'Très fort'}];
  const l=lvl[Math.min(sc,4)];
  const bar=document.getElementById('pwBar'),lbl=document.getElementById('pwLbl');
  if(bar){bar.style.width=v?l.w:'0';bar.style.background=l.c;}
  if(lbl){lbl.textContent=v?l.t:'Saisissez un mot de passe';lbl.style.color=v?l.c:'var(--gray400)';}
}
</script>
