...
<?php
$title='Souscription assurance'; $breadcrumb='Assurances · Nouvelle souscription';
$type_param=htmlspecialchars($_GET['type']??'habitation',ENT_QUOTES,'UTF-8');
$types=['habitation'=>'🏠 Assurance Habitation','auto'=>'🚗 Assurance Auto','vie'=>'💼 Assurance Vie','sante'=>'❤️ Assurance Santé','mobile'=>'📱 Assurance Mobile','voyage'=>'✈️ Assurance Voyage'];
$success=false;
if($_SERVER['REQUEST_METHOD']==='POST'&&Security::verifyCsrf($_POST['_csrf']??'')){
    $uid=Session::userId();
    $type=$_POST['type']??'habitation';
    $montant=10000.00;
    notify($uid,'Souscription en cours','Votre demande de souscription assurance est en cours d\'étude.','info');
    DB::insertInto('assurances',[
        'user_id'         => $uid,
        'type'            => $type,
        'numero_contrat'  => 'GTB-' . strtoupper($type) . '-' . date('Ymd') . '-' . rand(1000,9999),
        'compagnie'       => 'GTB Protect',
        'prime_mensuelle' => round($montant * 0.002, 2),
        'statut'          => 'en_attente',
...
$csrf=csrf_token();
?>
<div class="sec-head"><div><a href="index.php" style="font-size:.8rem;color:var(--gray400);text-decoration:none">← Retour aux assurances</a><h1 class="pg-title" style="margin-top:.4rem">Souscrire une assurance</h1></div></div>
<?php if($success):?>
<div style="background:rgba(0,198,122,.08);border:1px solid rgba(0,198,122,.2);border-radius:var(--r-xl);padding:2rem;text-align:center">
  ...
  <div style="font-family:'Sora',sans-serif;font-weight:800;font-size:1.1rem;color:var(--bnp-dark)">Demande enregistrée !</div>
  <div style="font-size:.85rem;color:var(--gray600);margin:.5rem 0 1.5rem">Un conseiller vous contactera dans 24h pour finaliser la souscription.</div>
  <a href="index.php" class="btn btn-primary">Retour aux assurances</a>
</div>
<?php else:?>
<div style="display:grid;grid-template-columns:1fr 280px;gap:1.5rem">
  <div class="card card-pad">
    <form method="POST">
      ...
      <div class="form-group"><label class="form-label">Type d'assurance</label>
        <select name="type" class="form-select" required>
          <?php foreach($types as $k=>$v):?><option value="<?php echo $k;?>" <?php echo $k===$type_param?'selected':'';?>><?php echo $v;?></option><?php endforeach;?>
        </select></div>
      <div class="form-row">
        <div class="form-group"><label class="form-label">Prénom</label><input class="form-input" value="<?php echo e($u['first_name']??'');?>" name="prenom" required/></div>
        <div class="form-group"><label class="form-label">Nom</label><input class="form-input" value="<?php echo e($u['last_name']??'');?>" name="nom" required/></div>
      </div>
      <div class="form-group"><label class="form-label">Objet à assurer</label><textarea class="form-textarea" name="description" placeholder="Décrivez ce que vous souhaitez assurer…" rows="3" required></textarea></div>
      <div class="form-group"><label class="form-label">Date de prise d'effet souhaitée</label><input class="form-input" type="date" name="date_effet" min="<?php echo date('Y-m-d');?>"/></div>
      <button type="submit" class="btn btn-primary" style="width:100%">Soumettre la demande →</button>
    </form>
  </div>
  <div class="card card-pad info-box">
    <div style="font-weight:700;margin-bottom:.5rem">ℹ️ Comment ça marche</div>
    <div style="line-height:1.8">1. Vous soumettez la demande<br>2. Un conseiller vous rappelle sous 24h<br>3. Signature électronique du contrat<br>4. Couverture immédiate</div>
  </div>
</div>
<?php endif;?>
